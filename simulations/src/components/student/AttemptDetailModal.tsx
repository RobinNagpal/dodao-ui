...
export default function AttemptDetailModal({ isOpen, onClose, attempt, onSaveEdit, isUpdating }: AttemptDetailModalProps) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm transition-opacity" onClick={handleClose} />

      {/* Modal */}
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div
          className="relative w-full sm:max-w-3xl max-h-[90vh] bg-white/95 backdrop-blur-md border border-blue-100/50 shadow-2xl overflow-hidden rounded-2xl transform transition-all"
        ...
        >
          {/* Close Button */}
          <button onClick={handleClose} className="absolute top-4 right-4 z-10 p-2 rounded-full bg-white/80 hover:bg-white shadow-md transition-colors">
            ...
            <div className="text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <h2 id="modal-title">Attempt {attempt.attemptNumber} Details</h2>
                  <Sparkles className="h-5 w-5 text-yellow-500 animate-pulse" />
                </div>
                <div className="flex items-center space-x-3 text-sm pr-8">
                  <div className="flex items-center space-x-2">
                    <div
          ...
          {/* Content */}
          <div className="overflow-y-auto max-h-[70vh] space-y-6 p-6">
            {/* Your Prompt */}
                ...
                {isEditing ? (
                  <div className="space-y-4">
                    <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-4 border border-purple-200/50">
                      <MarkdownEditor objectId={attempt.id} modelValue={editedResponse} onUpdate={setEditedResponse} maxHeight={400} />
                    </div>
