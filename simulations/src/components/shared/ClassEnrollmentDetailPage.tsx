...
import { Activity, ClipboardList, GraduationCap, Shield, Users } from 'lucide-react';
...
const TabNavigation: FC<TabNavigationProps> = ({ activeTab, onTabChange, userType }) => {
  const activeColor = userType === 'admin' ? 'emerald' : 'purple';
        ...
        <button
          ...
          className={`py-4 px-2 pb-2 relative font-semibold text-sm flex items-center space-x-2 transition-all duration-300 ${
            activeTab === 'student-attempts'
              ? `text-${activeColor}-600 bg-${activeColor}-50/50 rounded-t-lg after:absolute after:bottom-1 after:left-0 after:right-0 after:h-0.5 after:bg-${activeColor}-500`
              : `text-gray-600 hover:text-${activeColor}-600`
          }`}
        >
...
export default function ClassEnrollmentDetailPage({ caseStudyId, classEnrollmentId, userType }: ClassEnrollmentDetailPageProps) {
  ...
  const { renderAuthGuard } = useAuthGuard({
    allowedRoles: userType === 'admin' ? 'Admin' : ['Instructor', 'Admin'],
    loadingType: userType,
  ...
  const bgGradient =
    userType === 'admin' ? 'bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50' : 'bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50';
  ...
  return (
    <div className={`min-h-screen ${bgGradient}`}>
      <div className="fixed inset-0 overflow-hidden pointer-events-none">{bgElements}</div>

      {userType === 'admin' ? (
        <AdminNavbar title={studentsData?.className || 'Class Enrollment'} subtitle="Students Details" icon={<Shield className="h-8 w-8 text-white" />} />
      ) : (
        <InstructorNavbar
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 pt-6">
        ...
        {/* Tab Navigation */}
        <TabNavigation activeTab={activeTab} onTabChange={setActiveTab} userType={userType} />
      </div>
