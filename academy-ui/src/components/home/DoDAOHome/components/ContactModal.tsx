...
export function TextField({ id, label, type = 'text', onChange, ...props }: any) {
  return (
    <div>
      <label className="block text-sm/6 text-body">{label}</label>
      <div className="mt-2.5">
        <input
          ...
          {...props}
          onChange={onChange}
          className="block w-full rounded-md border-0 bg-surface-2 px-3.5 py-2 text-heading shadow-sm ring-1 ring-inset ring-border placeholder:text-muted focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm/6"
        />
      </div>
    </div>
  );
      ...
      <div className="p-6 text-left">
        <form action="#" method="POST" className="mx-auto max-w-xl" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <label htmlFor="message" className="block text-sm/6 text-body">
                Message
              </label>
              <div className="mt-2.5">
                <textarea
