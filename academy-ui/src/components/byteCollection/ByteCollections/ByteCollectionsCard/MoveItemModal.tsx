...
export default function MoveItemModal(props: MoveItemModalProps) {
  ...
  const [byteCollectionsResponse, setByteCollectionsResponse] = useState<ByteCollectionSummary[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      const response = await axios.get(`${getBaseUrl()}/api/${spaceId}/byte-collections`);
  ...
  const availableByteCollections = byteCollectionsResponse.filter((byteCollection) => byteCollection.id != byteCollectionId) || [];
