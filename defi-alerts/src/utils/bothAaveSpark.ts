...
export function getAaveSparkAprs(): () => Promise<MarketApr[]> {
  ...
  const getFlattenedAddresses = (
    addrs: FlattenedAddresses | Address
  ): Address[] =>
    typeof addrs === "object"
      ? (Object.values(addrs) as Address[])
      : ([addrs] as Address[]);

  // Fetch APRs for a single protocol on a given chain
  const getProtocolAprs = async (
    chainId: number,
    protocol: "aave" | "spark"
  ): Promise<MarketApr[]> => {
    ...
    const providerAddress =
      protocol === "aave"
        ? getFlattenedAddresses(AAVE_CONFIG_POOL_CONTRACT[chainId])[0]
        : SPARK_DATA_PROVIDER[chainId];
    const dataProviderAbi =
      protocol === "aave" ? PoolDataAddressAbi_Arbitrum : Pool_Abi_DataProvider;
  ...
  return async () => {
    const aaveChains = Object.keys(AAVE_CONFIG_POOL_CONTRACT).map((id) =>
      Number(id)
    );
    const sparkChains = Object.keys(SPARK_DATA_PROVIDER).map((id) =>
      Number(id)
    );

    const calls: Promise<MarketApr[]>[] = [];
    aaveChains.forEach((chainId) =>
      calls.push(getProtocolAprs(chainId, "aave"))
    );
    sparkChains.forEach((chainId) =>
      calls.push(getProtocolAprs(chainId, "spark"))
    );
