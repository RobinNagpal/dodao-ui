...
import { ChevronRight, Home, Bell, TrendingUp, ArrowLeft, ArrowRight, Wallet } from 'lucide-react';
...
export default function PersonalizedSetupPage() {
  ...
  const handleNext = () => {
    ...
    const alertType = localStorage.getItem('selectedPersonalizedAlertType') || 'compound';
  ...
  return (
    <div className="container max-w-6xl mx-auto px-2 py-8">
      ...
      <nav className="flex items-center text-sm mb-6">
        ...
        <ChevronRight size={14} className="mx-2 text-theme-muted" />
        <span className="text-primary-color font-medium">Personalized Setup</span>
      </nav>

      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2 text-theme-primary">Set Up Personalized Alerts</h1>
        <p className="text-theme-muted">Configure alerts based on your wallet activity.</p>
      </div>

      <Card className="mb-6 border-theme-primary bg-block">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg text-theme-primary">Enter Your Wallet Address</CardTitle>
          <CardDescription className="text-theme-muted">Write your wallet address to see your active positions and set up personalized alerts.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={handleNext} className="border text-primary-color hover-border-body">
