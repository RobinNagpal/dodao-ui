...
import { arbitrum, base, Chain, mainnet, mantle, optimism, polygon, scroll, sepolia, linea, ronin, unichain } from 'wagmi/chains';

export const supportedChains: readonly [Chain, ...Chain[]] = [sepolia, mainnet, polygon, arbitrum, base, optimism, scroll, mantle, linea, unichain, ronin];
...
export const useDefaultConfig: any = createConfig({
  chains: [mainnet, polygon, arbitrum, base, optimism, scroll, mantle, linea, ronin, unichain],
  transports: {
    [mainnet.id]: fallback([
      http(),
    ...
    [arbitrum.id]: fallback([http(), http('https://1rpc.io/arb'), http('https://arbitrum.meowrpc.com'), http('https://arbitrum-one.public.blastapi.io')]),
    [base.id]: fallback([
      http(),
    ...
    [linea.id]: fallback([http(), http('https://rpc.linea.build'), http('https://1rpc.io/linea'), http('https://linea.drpc.org')]),
    [ronin.id]: fallback([http(), http('https://api.roninchain.com/rpc'), http('https://ronin-rpc.publicnode.com'), http('https://rpc.ankr.com/ronin')]),
