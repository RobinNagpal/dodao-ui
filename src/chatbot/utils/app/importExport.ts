...
import { ExportFormatV1, ExportFormatV2, ExportFormatV3, ExportFormatV4, LatestExportFormat, SupportedExportFormats } from '@/chatbot/types/export';
import { FolderInterface } from '@/chatbot/types/folder';
import { Prompt } from '@/chatbot/types/prompt';

import { cleanConversationHistory } from '@/chatbot/utils/app/clean';
...
export const importData = (data: SupportedExportFormats): LatestExportFormat => {
  ...
  const newFolders: FolderInterface[] = [...oldFoldersParsed, ...folders].filter((folder, index, self) => index === self.findIndex((f) => f.id === folder.id));
  localStorage.setItem('folders', JSON.stringify(newFolders));

  const oldPrompts = localStorage.getItem('prompts');
  const oldPromptsParsed = oldPrompts ? JSON.parse(oldPrompts) : [];
  const newPrompts: Prompt[] = [...oldPromptsParsed, ...prompts].filter((prompt, index, self) => index === self.findIndex((p) => p.id === prompt.id));
