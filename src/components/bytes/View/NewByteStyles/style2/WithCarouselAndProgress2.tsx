...
import styles from './WithCarouselAndProgress2.module.scss';
import { AnimatePresence, motion } from 'framer-motion';
import StepIndicatorProgress from '../Progress/StepIndicatorProgress';
import CustomGradientButton from '@/components/core/buttons/GradientButton';

interface WithCarouselAndProgress2Props {
...
function WithCarouselAndProgress2({ viewByteHelper, step, byte, space }: WithCarouselAndProgress2Props) {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={`px-2 lg:px-4 py-5 sm:px-6 flex flex-col justify-between w-full ${styles.stepContent}`}
      >
        <div className={`px-2 lg:px-4 py-5 sm:px-6 flex flex-col justify-between w-full ${styles.stepContent}`}>
          ...
          <div className="mt-2">
            {isNotFirstStep && !isByteCompletedStep && (
              <CustomGradientButton onClick={() => viewByteHelper.goToPreviousStep(step)} className="float-left" backgroundColor="grey">
                Back
...
export default WithCarouselAndProgress2;
