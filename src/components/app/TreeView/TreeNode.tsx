    ...
    <div className="px-2 py-2">
      <div className="flex items-center cursor-pointer" onClick={handleClick}>
        {node.children && node.children.length > 0 ? (
          isOpen ? (
            <ChevronDownIcon className="h-5 w-4 mr-1" />
          ) : (
            <ChevronRightIcon className="h-5 w-4 mr-1" />
          )
        ) : null}
      ...
      {isOpen && node.children && (
        <div className="ml-2">
          {node.children.map((childNode, index) => (
            <TreeNode key={(childNode.component.key as string) || index} node={childNode} openNodes={openNodes} setOpenNodes={setOpenNodes} level={level + 1} />
          ))}
        </div>
      )}
    </div>
