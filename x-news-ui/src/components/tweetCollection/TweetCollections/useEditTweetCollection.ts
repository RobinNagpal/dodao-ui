...
export function useEditTweetCollection({
  tweetCollection: tweetCollectionProp,
}: UseEditTweetCollectionArgs): UseEditTweetCollectionType {
  const redirectPath = "/";
  ...
  const upsertTweetCollection = async () => {
    ...
    if (
      !tweetCollection.name.trim() ||
      !tweetCollection.description.trim() ||
      !tweetCollection.handles.length
    ) {
      return;
    }
    try {
      ...
      if (tweetCollectionProp?.id) {
        await putData(
          `${getBaseUrl()}/api/tweet-collections/${tweetCollectionProp.id}`,
          {
