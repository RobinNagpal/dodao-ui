...
export default function AddNewItemButton({ isAdmin, tweetCollection }: { isAdmin: boolean; tweetCollection: TweetCollectionSummary }) {
  ...
  const handleAddHandleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setUpserting(true);
    const response = await putData(`${getBaseUrl()}/api/tweet-collections/${tweetCollection.id}`, {
  ...
  return (
    <>
      ...
      <SingleSectionModal open={showCreateModal} onClose={() => setShowCreateModal(false)} title={'Add New Handle'} showSemiTransparentBg={true}>
        <div className="text-left py-4">
          <form onSubmit={handleAddHandleSubmit}>
            <Input id="handle" modelValue={handle} placeholder="@" onUpdate={(e) => (e ? setHandle(e.toString()) : setHandle(''))} required label={'Handle'} />
            <div className="w-full flex justify-center">
              <Button type="submit" primary variant={'contained'} className="mt-4" loading={upserting} disabled={upserting}>
                Add Handle
              </Button>
            </div>
          </form>
