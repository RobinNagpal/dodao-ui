...
const refreshURL =
  process.env.NEXT_PUBLIC_TWEET_COLLECTION_REFRESHER_URL?.toString() || "";
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        alert(
          `Unable to refresh the collection "${tweetCollection.name}". Error: ${errorData.error}`
        );
      }
    } catch (error) {
