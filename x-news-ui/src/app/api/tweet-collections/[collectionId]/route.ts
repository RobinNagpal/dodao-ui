...
async function putHandler(req: NextRequest, { params }: { params: Promise<{ collectionId: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to update tweet collection.' }, { status: 500 });
  }
}
