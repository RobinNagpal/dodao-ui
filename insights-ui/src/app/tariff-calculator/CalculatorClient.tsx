...
function CodePicker({ onSelect, manualCode, setManualCode, manualError, onManualSubmit }: CodePickerProps): JSX.Element {
  return (
    <div className="rounded-xl bg-bg border border-surface shadow-2xl p-4 sm:p-6 lg:p-8">
      ...
      <form onSubmit={onManualSubmit} className="mt-6 flex flex-col sm:flex-row gap-3">
        <input
          ...
          aria-label="HTS code"
          className="flex-1 h-11 rounded-lg border border-border bg-well px-4 text-sm text-heading placeholder-muted-3 font-mono focus:outline-none focus:ring-2 focus:ring-amber-300/40 focus:border-amber-300/60"
        />
        <button
          type="submit"
...
function ShipmentForm({ form, updateForm, submitting, hideQuantityInputs, primaryUoms, onSubmit, error }: ShipmentFormProps): JSX.Element {
  return (
    <form onSubmit={onSubmit} className="rounded-xl bg-bg border border-surface p-4 sm:p-6 shadow-lg space-y-4">
      ...
      <Field label="Shipment Value (USD)" htmlFor="shipmentValueUsd">
        <input
          ...
          className="block w-full rounded-md border border-border bg-well px-3 py-2 text-sm text-heading shadow-xs focus:outline-none focus:ring-2 focus:ring-amber-300/40 focus:border-amber-300/60"
        />
      </Field>
            ...
            <Field label="UOM" htmlFor="unitOfMeasure">
              {primaryUoms.length === 1 ? (
                <input
                  ...
                  value={primaryUoms[0]}
                  readOnly
                  className="block w-full rounded-md border border-border bg-well px-3 py-2 text-sm text-heading shadow-xs uppercase opacity-80"
                />
              ) : primaryUoms.length > 1 ? (
                <select
                  id="unitOfMeasure"
                  ...
                  className="block w-full rounded-md border border-border bg-well px-3 py-2 text-sm text-heading shadow-xs uppercase focus:outline-none focus:ring-2 focus:ring-amber-300/40 focus:border-amber-300/60"
                >
                  {primaryUoms.map((u) => (
...
function ResultPanel({ result, submitting, onToggleExclusion }: ResultPanelProps): JSX.Element {
  ...
  return (
    <>
      <div className="rounded-xl bg-bg border border-surface p-4 sm:p-6 shadow-lg">
      ...
      <div className="rounded-xl bg-bg border border-surface overflow-hidden shadow-lg">
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[480px]">
            <thead>
              <tr className="border-b border-surface text-left text-xs uppercase tracking-wide opacity-60">
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }): JSX.Element {
  return (
    <div className={`flex justify-between ${bold ? 'font-semibold border-t border-surface pt-2 mt-2 text-amber-200' : ''}`}>
      <dt>{label}</dt>
