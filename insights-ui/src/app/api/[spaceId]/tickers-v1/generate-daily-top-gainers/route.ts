...
async function getHandler(req: NextRequest, { params }: { params: Promise<{ spaceId: string }> }): Promise<GenerateStockMoversResponse> {
  ...
  const saveResult = await processAndSaveStockMovers(data, spaceId, DailyMoverType.GAINER);

  // Get all saved movers in one query
  const savedSymbols = saveResult.savedStocks.map((stock) => stock.symbol);
  const movers = await prisma.dailyTopGainer.findMany({
    where: {
      spaceId,
  ...
  for (const stock of data.stocks) {
    ...
    if (mover) {
      try {
        await processDailyMover(spaceId, stock, mover.id, DailyMoverType.GAINER);
      } catch (error) {
