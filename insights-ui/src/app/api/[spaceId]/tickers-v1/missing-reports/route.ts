...
import { getTickersWithMissingReports } from '@/utils/missing-reports-utils';
...
): Promise<TickerWithMissingReportInfo[]> => {
  const { spaceId } = await params;
  return getTickersWithMissingReports(spaceId);
};

export const GET = withLoggedInAdmin<TickerWithMissingReportInfo[]>(getHandler);
