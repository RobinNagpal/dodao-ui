...
function computeReportStatus(hasData: boolean, step: EtfReportType, flag: RegenerateFlagKey, latestRequest: LatestRequestSummary | undefined): EtfReportStatus {
  ...
  const wasRequested = Boolean(latestRequest[flag]);
  ...
  return {
    etfs: supportedEtfs.map((e) => {
      ...
      const reportStatuses: EtfReportStatuses = {
        ...
        // Competition data lives in a dedicated table that is not yet generated by the
        // pipeline. Until the backend lands, we report status purely from the
        // generation-request flags so the admin UI can still show queued / failed state.
        competition: computeReportStatus(false, EtfReportType.COMPETITION, 'regenerateCompetition', latestRequest),
