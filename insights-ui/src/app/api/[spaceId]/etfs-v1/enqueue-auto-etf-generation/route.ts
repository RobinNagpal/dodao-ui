 ...
 * excludes ETFs) it returns immediately without touching Claude; otherwise, when no
 * batch is open, the mode's cooldown has elapsed, and the shared usage gates pass,
 * it creates one batch (the mode's batch size) of full-report requests for ETFs
 * missing their reports (US first, then Canada, then other). The existing ~3-min
 * `generate-etf-v1-request` processor then generates them normally.
 */
