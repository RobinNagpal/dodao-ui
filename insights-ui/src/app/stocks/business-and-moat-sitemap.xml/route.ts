...
async function generateBusinessAndMoatUrls(): Promise<SiteMapUrl[]> {
  const urls: SiteMapUrl[] = [];

  const records = await prisma.tickerV1CategoryAnalysisResult.findMany({
    where: {
  ...
  for (const record of records) {
    const url = `/stocks/${record.ticker.exchange}/${record.ticker.symbol}/business-and-moat`;
    urls.push({
      url,
      changefreq: 'weekly',
      priority: 0.6,
...
async function GET(req: NextRequest): Promise<NextResponse<Buffer>> {
  ...
  try {
    ...
    console.error('Error generating business & moat sitemap:', error);
