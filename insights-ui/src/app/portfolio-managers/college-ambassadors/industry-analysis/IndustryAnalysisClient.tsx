...
export default function IndustryAnalysisClient({ industries, prompts }: IndustryAnalysisClientProps) {
  ...
  const renderPromptContent = (prompt: PromptConfig) => {
    if (!selectedIndustry) {
      return <p className="text-muted-2 italic">Please select an industry first</p>;
    }
    ...
    return (
      <div className="space-y-4" onClick={(e) => e.stopPropagation()}>
        ...
        {dbPrompt.excerpt && <div className="text-sm text-muted-2 italic mb-4">{dbPrompt.excerpt}</div>}

        <div className="text-muted-1 markdown markdown-body bg-surface rounded-lg p-6" dangerouslySetInnerHTML={{ __html: parseMarkdown(finalPrompt) }} />
      </div>
    );
  };
