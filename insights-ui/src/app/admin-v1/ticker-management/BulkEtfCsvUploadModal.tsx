...
interface BulkEtfCsvUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

const REQUIRED_HEADERS = ['symbol', 'name', 'exchange'] as const;

/** ---------- Component ---------- */

export default function BulkEtfCsvUploadModal({ isOpen, onClose, onSuccess }: BulkEtfCsvUploadModalProps): JSX.Element {
  const [loading, setLoading] = useState(false);
  const [parseError, setParseError] = useState<string>('');
  const [result, setResult] = useState<BulkEtfCsvResponse | null>(null);
  ...
  const submitRows = async (rows: EtfCsvRow[]): Promise<void> => {
    const etfs = rows
      .filter((r) => r.symbol && r.name && r.exchange)
      .map((r) => ({
        symbol: (r.symbol ?? '').trim().toUpperCase(),
        name: (r.name ?? '').trim(),
        exchange: (r.exchange ?? '').trim().toUpperCase(),
        provider: (r.provider ?? '').trim() || undefined,
        segment: (r.segment ?? '').trim() || undefined,
      }));
    ...
    setLoading(true);
    try {
      const res = await fetch(`${getBaseUrl()}/api/${KoalaGainsSpaceId}/etfs/bulk-csv`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ etfs }),
      });

      const data: BulkEtfCsvResponse = await res.json();
      setResult(data);
      ...
      setParseError(err instanceof Error ? err.message : 'Failed to upload ETFs');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SingleSectionModal open={isOpen} onClose={handleClose} title="Bulk Upload ETFs from CSV">
      <div className="space-y-4 p-2">
        ...
        <div className="rounded-md bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-3 text-sm">
          ...
          <code className="text-xs text-gray-600 dark:text-gray-400">symbol, name, exchange</code>
          <p className="font-medium text-gray-700 dark:text-gray-300 mt-2 mb-1">Optional columns:</p>
          <code className="text-xs text-gray-600 dark:text-gray-400">provider, segment</code>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Duplicate ETFs (same symbol + exchange) will be reported as errors.</p>
        </div>
          ...
          <div className="space-y-3">
            {result.addedCount > 0 && (
              <div className="rounded-md bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 p-3 text-sm text-green-700 dark:text-green-400">
                ✓ Successfully added <strong>{result.addedCount}</strong> ETF{result.addedCount !== 1 ? 's' : ''}.
              </div>
            )}
            ...
            {result.errorRows.length > 0 && (
              <div className="rounded-md bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 p-3 text-sm">
                <p className="font-medium text-yellow-800 dark:text-yellow-300 mb-2">
                  {result.errorRows.length} ETF{result.errorRows.length !== 1 ? 's' : ''} could not be added:
                </p>
