    ...
    <>
      <div className="bg-surface -mx-6 px-6 py-6 mb-6 border-b border-border">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-heading">Stock Scenarios</h1>
            <p className="text-muted mt-1">Manage the dated playbook of stock-market scenarios, scoped per country.</p>
          </div>
              ...
              {scenarios.map((s) => (
                <tr key={s.id} className="border-t border-border hover:bg-surface">
                  ...
                  <td className="px-3 py-2 text-heading">
                    <Link href={`/stock-scenarios/${s.slug}`} className="hover:text-link" target="_blank">
                      {s.title}
                    </Link>
                  </td>
                  <td className="px-3 py-2 text-muted">
                    <div className="flex flex-wrap gap-1">
                      {s.countries.map((c) => (
                        <span key={c} className="text-[10px] uppercase tracking-wide text-muted bg-bg border border-border rounded px-1.5 py-0.5">
                          {c}
                        </span>
                      ))}
                    </div>
