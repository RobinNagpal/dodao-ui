          ...
          <>
            {detail && detail.countries.length > 0 && (
              <p className="text-xs text-muted">
                This scenario is scoped to: <span className="text-body font-medium">{detail.countries.join(', ')}</span>. Links on any other country&apos;s
            ...
            <div className="rounded-lg border border-border bg-bg p-3 space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                ...
                />
                <label className="flex flex-col gap-1 text-sm">
                  <span className="text-muted">Expected price change % (-100 to 2000)</span>
                  <input
                    type="number"
                    min={-100}
                    max={2000}
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-muted">How much is already priced in</span>
                <select
                  className="bg-bg border border-border rounded px-2 py-1.5 text-sm text-heading"
                ...
                {group.items.length === 0 ? (
                  ...
                  <ul className="space-y-1">
                    {group.items.map((link) => (
                      <li key={`${link.symbol}-${link.exchange}-${link.role}`} className="bg-bg border border-border rounded px-2 py-1.5 text-sm">
                        <div className="flex items-center justify-between">
                          <span>
                            ...
                            <span className="ml-2 text-[10px] uppercase tracking-wide text-muted bg-surface border border-border rounded px-1.5 py-0.5">
