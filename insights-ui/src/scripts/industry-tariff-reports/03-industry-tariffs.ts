...
function getTopTradingCountriesPrompt(industryName: string, date: string) {
  return `
Identify the top 5 countries by trading volume with the U.S. for the ${industryName} industry as of ${date}.
...
async function getTopTradingCountries(industryName: string, date: string): Promise<string[]> {
  const prompt = getTopTradingCountriesPrompt(industryName, date);
...
function getTariffUpdatesForIndustryPrompt(industryName: string, date: string, headings: IndustryAreasWrapper, country: string) {
  const prompt = `
  As of today (${getDateAsMonthDDYYYYFormat(date)}), I want to know about the new or recent tariffs added for the ${industryName} industry for ${country}.
  Make sure to verify all the new tariffs added for ${industryName} industry and as of ${getDateAsMonthDDYYYYFormat(
    date
...
async function getTariffUpdatesForIndustry(
  ...
  if (countryName) {
    ...
    countriesToProcess = await getTopTradingCountries(ctx.industryName, date);
  }
  ...
  for (const country of countriesToProcess) {
    const prompt = getTariffUpdatesForIndustryPrompt(ctx.industryName, date, headings, country);
