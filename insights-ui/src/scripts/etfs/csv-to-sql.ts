/**
 * Convert an ETF screener CSV/TSV export (e.g. `public/uk-etfs.csv`) into a
 * `.sql` file with raw INSERTs for `etfs`, `etf_financial_info`, and
 ...
 *
 * Usage:
 *   yarn etfs:csv-to-sql                       # reads public/uk-etfs.csv, writes public/uk-etfs.sql
 *   yarn etfs:csv-to-sql --in <path> --out <path>
 *
 * Notes on input shape:
 *   - The "Symbol" column is expected to be `EXCHANGE:TICKER` (e.g. `LON:0LO6`,
 *     `TSX:VFV`, `NEO:FEQT`). The prefix sets the exchange code (`LON` → `LSE`
 *     for the London Stock Exchange, `CBOE` → `NEO` for Cboe Canada). Bare-ticker
 *     rows fall back to the "Exchange" column ("London Stock Exchange" → LSE,
 *     "Toronto Stock Exchange" → TSX, etc.).
...
import { CanadaExchanges, UKExchanges, USExchanges } from '@/utils/countryExchangeUtils';
import { SPACE_ID, parseArgs } from './lib';

const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../..');
const DEFAULT_IN = path.join(REPO_ROOT, 'public', 'uk-etfs.csv');
const DEFAULT_OUT = path.join(REPO_ROOT, 'public', 'uk-etfs.sql');
...
const EXCHANGE_NAME_TO_CODE: Record<string, string> = {
  'London Stock Exchange': UKExchanges.LSE,
 ...
 * `LON` is the London Stock Exchange — stored as `LSE` in our schema.
...
async function main(): Promise<void> {
  ...
  const inPath = typeof inArg === 'string' ? path.resolve(inArg) : DEFAULT_IN;
