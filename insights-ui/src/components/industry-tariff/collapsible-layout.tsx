...
export default function CollapsibleLayout({ children, basePath, lastModified }: CollapsibleLayoutProps) {
    ...
    <div className="relative">
      {!isSidebarOpen && (
        <button
          ...
          className="fixed bottom-6 left-6 z-50 bg-primary text-primary-text px-4 py-3 rounded-full shadow-lg transition-all duration-200 hover:scale-105 flex items-center gap-2"
      ...
      <div className="flex min-h-[calc(100vh-10rem)] overflow-hidden rounded-lg border border-color background-color shadow-lg">
        <div className={cn('transition-all duration-300 ease-in-out overflow-hidden', isSidebarOpen ? 'w-72' : 'w-0')}>
          <div className="w-72 h-full">
            <ReportLeftNavigation basePath={basePath} onToggle={toggleSidebar} showToggle={true} lastModified={lastModified} />
          </div>
        </div>
