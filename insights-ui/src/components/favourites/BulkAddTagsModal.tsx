      ...
      <div className="p-4 space-y-4 text-left">
        <div className="bg-surface p-3 rounded-lg">
          <p className="text-sm text-muted mb-3">
            You are about to update tags for <span className="font-semibold text-heading">{selectedFavouriteIds.size}</span> selected{' '}
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wide text-muted mb-1.5">Select Tags</label>
            <div className="space-y-0.5 max-h-52 overflow-y-auto bg-bg rounded-md p-1.5">
              {tags.length === 0 ? (
                <p className="text-muted text-sm px-2 py-1.5">No tags available. Create tags first.</p>
              ) : (
                <Checkboxes
                  items={tags.map(
                    (tag): CheckboxItem => ({
