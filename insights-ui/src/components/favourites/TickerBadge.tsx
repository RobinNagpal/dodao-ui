...
export default function TickerBadge({
  ...
  const content = (
    <div className="flex items-center gap-1.5 px-2 py-1 border border-gray-600 rounded hover:border-gray-500 transition-colors">
      {showScore && (
        <span className={`${textColorClass} px-1 rounded ${bgColorClass} bg-opacity-15 text-xs font-mono`}>
          {score}/25
        </span>
      )}
      <span className="text-xs font-medium bg-[#4F46E5] text-white px-1.5 py-0.5 rounded shrink-0">
        {ticker.symbol}
      </span>
      {showName && (
        <span className="text-xs text-white truncate max-w-[150px]">{ticker.name}</span>
      )}
      {onRemove && (
        <button
          onClick={(e) => {
  ...
  if (linkToStock) {
    return (
      <Link href={`/stocks/${ticker.exchange}/${ticker.symbol}`}>
        {content}
      </Link>
    );
  }
