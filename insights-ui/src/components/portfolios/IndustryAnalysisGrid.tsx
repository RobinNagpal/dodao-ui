  ...
  if (!industriesByCountry || industriesByCountry.length === 0) {
    return (
      <div className="bg-bg rounded-lg p-8 text-center border border-surface">
        <p className="text-muted-2">No industry analysis data available.</p>
      </div>
    );
  }
      ...
      {industriesByCountry.map((item) => {
        ...
        return (
          <Link
            ...
            className="bg-bg rounded-lg p-5 transition-all border border-surface hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/10 group"
          >
            <div className="flex flex-col h-full">
              ...
              {/* Stats */}
              <div className="flex items-center gap-4 text-xs pt-3 border-t border-surface">
                <div className="flex items-center gap-1">
                  <span className="px-2 py-0.5 bg-surface text-muted-1 rounded">{countryCode}</span>
                </div>
                {item.favoriteCount > 0 && (
                  <div className="flex items-center gap-1">
                    <span className="text-yellow-400">⭐</span>
                    <span className="text-muted-1">{item.favoriteCount}</span>
                  </div>
                )}
                {item.notesCount > 0 && (
                  <div className="flex items-center gap-1">
                    <span className="text-blue-400">📝</span>
                    <span className="text-muted-1">{item.notesCount}</span>
                  </div>
                )}
