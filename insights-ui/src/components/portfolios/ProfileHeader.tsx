...
export default function ProfileHeader({ profile, portfolioManagerId }: ProfileHeaderProps) {
  ...
  return (
    <div className="bg-bg rounded-2xl overflow-hidden mb-8 border border-surface">
      <div className="flex flex-col md:flex-row gap-8 p-8">
        ...
        <div className="flex-shrink-0">
          ...
          {profile.profileImageUrl ? (
            ...
            <div className="w-48 h-56 bg-surface-2 rounded-xl flex items-center justify-center mb-6">
              <UserIcon className="w-24 h-24 text-muted-2" />
            </div>
          )}
          ...
          <div className="space-y-3 w-48">
            {profile.country && (
              <div className="flex items-center gap-2 text-xs text-muted-1 bg-surface px-3 py-2 rounded-lg">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Summary */}
          <p className="text-muted-1 text-sm leading-relaxed mb-5 mt-6">{profile.summary}</p>

          {/* Detailed Description */}
          {profile.detailedDescription && (
            <div
              className="text-muted-1 text-sm leading-relaxed markdown markdown-body"
