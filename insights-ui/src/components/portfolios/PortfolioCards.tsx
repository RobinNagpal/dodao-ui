  ...
  const renderPortfolioCard = (portfolio: PortfolioWithTickers) => {
    ...
    return (
      <Link
        ...
        className=" bg-bg rounded-lg p-6 transition-all block border border-surface hover:border-blue-500"
      >
        <div className="mb-4">
          <h3 className="text-xl font-semibold text-blue-400 group-hover:text-blue-300 transition-colors mb-2">{portfolio.name}</h3>
          <p className="text-muted-1 text-sm mb-3 line-clamp-2">{portfolio.summary}</p>
        </div>

        <div className="space-y-3">
          <div className="flex justify-between items-center text-sm">
            <span className="text-muted-2">Holdings:</span>
            <span className="text-heading font-medium">{portfolio.portfolioTickers?.length || 0}</span>
          </div>

          <div className="flex justify-between items-center text-sm">
            <span className="text-muted-2">Total Allocation:</span>
          ...
          {topHoldings.length > 0 && (
            <div className="pt-3 border-t border-border">
              <div className="text-sm text-muted-2 mb-2">Top Holdings:</div>
              <div className="space-y-2">
                {topHoldings.map((ticker) => (
                  <div key={ticker.id} className="flex justify-between items-center">
                    ...
                    <span className="text-muted-2 text-sm font-medium ml-2">{ticker.allocation}%</span>
                  </div>
                ))}
                {(portfolio.portfolioTickers?.length || 0) > 3 && (
                  <div className="text-sm text-muted-3">+{(portfolio.portfolioTickers?.length || 0) - 3} more</div>
                )}
              </div>
            </div>
          )}
