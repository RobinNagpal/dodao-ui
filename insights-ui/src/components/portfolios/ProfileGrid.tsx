    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {profiles.map((profile) => (
        <Link
          ...
          className="group bg-bg rounded-2xl overflow-hidden transition-all flex gap-6 p-6 border border-surface hover:border-blue-500"
        >
          ...
          <div className="flex-shrink-0">
            {profile.profileImageUrl ? (
              ...
              <div className="w-40 h-48 bg-surface-2 rounded-xl flex items-center justify-center">
                <UserIcon className="w-16 h-16 text-muted-2" />
              </div>
            )}
          </div>
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            {/* Summary */}
            <p className="text-muted-1 text-sm leading-relaxed mb-4 line-clamp-4 flex-1">{profile.summary}</p>

            {/* Bottom Row - Country and View Profile */}
            <div className="flex items-center justify-between gap-3 mt-auto">
              {profile.country && (
                <div className="flex items-center text-sm text-muted-2">
