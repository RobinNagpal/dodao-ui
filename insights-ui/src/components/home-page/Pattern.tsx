    ...
    <svg aria-hidden="true" width={width} height={height} {...props}>
      <defs>
        ...
        <symbol id={`${id}-1`} width={size} height={size}>
          ...
          <rect className="fill-blue-600" width={size / 2} height={size / 2} x={size / 4} y={size / 4} />
        </symbol>
      </defs>
      {pattern.map((row, rowIndex) =>
        row.map((shape, columnIndex) => (
          <use key={`${rowIndex}-${columnIndex}`} href={`#${id}-${shape}`} x={columnIndex * size + columnIndex * gapX} y={rowIndex * size + rowIndex * gapY} />
        ))
      )}
    </svg>
  );
