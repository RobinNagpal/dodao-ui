...
State whether the performance profile is **Strong**, **Mixed**, or **Weak**. Include 3–5 decision-useful numbers (absolute + relative). For cyclical asset classes (commodities, precious metals, single-country, sector-thematic), do not assign **Strong** on a `1Y`–`3Y` surge alone — weigh the `10Y` / `15Y` / `20Y` record first, so two funds riding the same macro move (e.g. a gold fund and a broad-commodity fund) do not land on opposite verdicts. End with one plain-English takeaway.
...
- **Translate beta / duration / RSI into practical terms — and only when the metric is meaningful.** Match the wording to the beta value. For **beta ≥ 1** it amplifies (`beta 1.17` → "expect ~17% more than the market — a `-20%` S&P drop usually puts this fund nearer `-23%`"). For **beta < 1** it dampens, so never call it "amplification" (`beta 0.57` → "moves only about 57% as much as the market — a `-20%` S&P drop usually puts this fund nearer `-11%`"). For **near-zero-beta, low-correlation asset classes — gold / precious metals, broad commodities, currency, managed futures** — do NOT force a "`-20%` S&P → this fund" sentence at all; beta is statistical noise there, so just state that the fund moves largely independently of equities. `duration 6 years` → "expect roughly a `-6%` price hit per `1 pp` rise in rates." Bare metric quotes without translation add nothing a retail reader can act on.
...
- You translated **beta / duration / leverage multiplier** into practical terms at least once (not just the number) — and you did NOT label a beta below `1.0` an "amplification", nor force a beta→S&P-move sentence onto a gold / commodity / currency fund.
