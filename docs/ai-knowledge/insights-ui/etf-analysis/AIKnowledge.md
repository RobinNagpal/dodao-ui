...
ETF market scenarios live in the `EtfScenario` Prisma model (see `insights-ui/prisma/schema.prisma`) and are managed through the admin UI at `/admin-v1/etf-scenarios` — there is no longer a markdown catalog or implementation checklist. For drafting a new scenario (scratch-file workflow under `/tmp/scenarios/etfs/<slug>.md` and the required content template), see [`../scenario-authoring.md`](../scenario-authoring.md).
