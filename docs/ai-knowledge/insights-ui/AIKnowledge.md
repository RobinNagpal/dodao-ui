...
- **[automated-report-generation.md](automated-report-generation.md)** — Four CLI scripts (`stocks:prompt`, `stocks:save`, `etfs:prompt`, `etfs:save`) that let Claude drive the full 7-ETF / 8-stock report-generation loop: setup, report order, per-type response JSON shapes, run-time gotchas (JSON-escape quotes, partial MOR data on first prompt, `final-summary` must be last), and the end-to-end copy-paste skeleton.
