...
- [ ] **Failed-indexed-pages pattern audit (site-wide)** — pull every URL flagged by Google Search Console as "Crawled — currently not indexed", "Discovered — currently not indexed", "Duplicate without user-selected canonical", "Duplicate, Google chose different canonical", "Alternate page with proper canonical tag", and "Soft 404" across every KoalaGains sitemap (stocks per-section, ETFs, ETF categories/countries/asset-classes/groups/providers, daily-top-movers, tariffs, scenarios, trends, crowd-funding, etfs sitemap-index children). Bucket each URL by failure reason × surface × sitemap, then cluster by root-cause pattern (thin/templated body, near-duplicate copy across siblings, weak/missing canonical, render-blocked content, missing JSON-LD, slow CWV, missing internal links, sitemap hygiene). Output: a single dashboard / markdown report under `docs/insights-ui/seo/` enumerating each pattern with affected URL counts, representative examples, a one-line root cause, and the targeted fix; per-pattern owner + fix linked to the relevant per-surface task (e.g. line 14 `business-and-moat-sitemap.xml`, daily-top-movers dedupe, tariff sitemap hygiene). Stand up a weekly recurring job that re-runs the GSC pull, diffs against the prior week, and posts the delta + new patterns to admin; a pattern only closes when its bucket shrinks 2 consecutive weeks. Open: GSC API access via service account vs scheduled CSV upload; whether to persist the per-URL failure history in a `SeoIndexStatus` table so we can correlate with `isComplete` / publish date / report-refresh runs.
