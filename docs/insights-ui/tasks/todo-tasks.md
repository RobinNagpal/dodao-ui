...
- [ ] **SEO — "Crawled — currently not indexed" on main `/stocks/{exchange}/{ticker}` pages** — ~95 main stock URLs failed the most recent GSC revalidation (out of ~15k flagged + ~682 revalidation failures total). 39-URL teardown + hypothesised root causes in [`../stocks-crawled-not-indexed-pattern-analysis.md`](../stocks-crawled-not-indexed-pattern-analysis.md): two distinct failing slices — (a) data-poor small/micro-cap Healthcare Biopharma + non-US microcaps clustered at ~2.3–2.5k visible words (Healthcare Biopharma 11/39, non-US 11/39, every Korean URL = Biopharma microcap), and (b) US large/mid-cap names in competitive query landscapes (BURL, NVR) where pages produce 3–8k words but compete head-on with Yahoo / MarketWatch / Seeking Alpha. The cron-batch `datePublished` fingerprint is sharp but is a *plausible weak signal* rather than a confirmed Google ranking factor (Google has never publicly documented `datePublished` precision as an indexing input). The HTML-weight finding is also weaker than initially framed — 92.5% of every page is the Next.js RSC streaming payload (analysis text shipped twice for hydration), 0 bytes of inline CSS, charts not inlined. Next steps before writing code: pull full failed-revalidation list and bucket by `(exchange, industryKey, ageInDays, visibleWords, hasMgmtTeam)`; run same teardown on an indexed-control sample to find the actual divider; then decide priority order across (a) backfill Management Team section on pre-June-12 reports, (b) strip trailing whitespace from industry names ("Healthcare: Technology & Equipment " hits 5 URLs in the sample alone), (c) per-ticker hero / Article-schema image, (d) verify `TickerV1.updatedAt` is bumped on every category-report edit so sitemap `lastmod` stays honest, (e) raise internal links to non-US country pages, (f) (low priority) round `datePublished` to date-only.
