...
### Tariff calculator — niche research via weak-competitor mining

> Hypothesis: most SEOs win calculator niches (GPA calculator, mortgage calculator, sales-tax calculator, etc.) by mining long-tail keywords where **low-DR, high-traffic, relatively new** competitors already rank — then publishing tighter content / a better calculator surface that out-covers them. We already have a working calculator at <https://koalagains.com/tariff-calculator>; this task figures out *which* keywords to chase so it lands on page 1 for queries we can realistically win.
>
> Research-only task. The deliverable is a single doc at `docs/insights-ui/tariffs/tariff-calculator-niche-research.md`. No code in this task — follow-up PRs ship the Tier 1 wins.

- [ ] **Seed long-tail discovery from big-platform mining (Ahrefs Site Explorer)** — feed Quora, Reddit, and Medium into Site Explorer one at a time. In **Organic Keywords** apply: `Lowest DR in Top 5 ≤ 5`, `Word Count ≥ 4`, `Location = United States` (then repeat for the next priority country once US is done). In the keyword filter, include any of: `tariff`, `duty`, `import`, `customs`, `hts`, `htsus`, `landed cost`, `section 301`, `section 232`, `ieepa`, `china tariff`, `mexico tariff`, `canada tariff`, `de minimis`. Export the full keyword list and tag each row by intent bucket (calc-question, rate-lookup, country-pair, section-specific, de-minimis, methodology).
- [ ] **Question-stem pass on the same platforms** — re-run with intent stems `how`, `what`, `why`, `does`, `best`, `can`, `is it` plus the same calculator/tariff keyword filter. These usually surface informational long-tails ("how much is the tariff on \<X\> from China", "what is section 301 tariff on \<Y\>", "best free tariff calculator", "does de minimis still apply to China shipments", "can I import \<X\> from \<country\> duty-free"). Merge with the previous list and dedupe.
- [ ] **Per-keyword SERP teardown** — Google each candidate keyword. For the top 10 results record: ranker URL, DR, estimated monthly organic traffic (Ahrefs / Semrush), domain age (whois), content type (blog post, calculator, forum thread, government PDF, freight-forwarder marketing page), word count, and whether they offer a *working* calculator vs static text. Flag every query where ≥ 2 of the top 10 have DR ≤ 30 **and** traffic > 500/mo — those are the achievable wins. The whole point of this exercise: only chase keywords where a weak site is already ranking, because that proves the keyword is rankable without DR-50+ authority.
- [ ] **Adjacent-calculator-niche calibration** — study how 1–2 weak winners in established calculator niches (GPA calculator, mortgage calculator, sales-tax calculator, paycheck calculator) won their long-tails: on-page content depth, FAQ length, schema markup (`WebApplication` + `FAQPage` + `HowTo`), internal-link density, backlink profile size, freshness signals (dated "Updated YYYY-MM" stamps), embedded examples. Note which of those tactics translate to `/tariff-calculator`.
- [ ] **Tier the keyword list** — produce a Tier 1 / Tier 2 / Tier 3 map:
  - **Tier 1 (ship in days):** long-tail questions a weak site is already winning that we can capture by adding a focused FAQ block, methodology section, or worked example inside the existing `/tariff-calculator` page.
  - **Tier 2 (ship in weeks):** queries that warrant a dedicated programmatic landing variant reusing the calculator engine — `/tariff-calculator/china`, `/tariff-calculator/mexico`, `/tariff-calculator/section-301`, `/tariff-calculator/section-232`, `/tariff-calculator/de-minimis`, `/tariff-calculator/hts-chapter-XX`. Each variant pre-fills calculator state and renders unique per-page content (current rate snapshot, recent changes, top HS codes for that origin/section, links to relevant tariff reports).
  - **Tier 3 (long game / deprioritised):** head terms where DR > 50 dominates the SERP — note them but don't plan work.
- [ ] **Map Tier 1 + Tier 2 keywords onto KoalaGains surfaces** — for each keyword, specify exactly where it lives (in-page section vs landing variant vs `/blog` post that internal-links back), the target angle, target word count, and the **unique data the page can show that the current weak ranker can't**: live HTS rate from our `tariff-calculator` engine, Section 232 / 301 / IEEPA overlays, country selector, dated last-updated stamp, link into the matching tariff report. The differentiation hook matters — Google won't rank us above a DR-5 site just because we copied its outline.
- [ ] **Internal-link + schema plan** — sketch the link graph: home → `/tariff-calculator`, `/tariff-reports` → `/tariff-calculator`, every country page → its `/tariff-calculator/<country>` variant, every Tier 1 blog post → `/tariff-calculator`. List the JSON-LD blocks each surface needs (`WebApplication`, `FAQPage`, `BreadcrumbList`, `HowTo` for worked-example pages) so SEO PRs ship them consistently.
- [ ] **Deliverable** — single research doc at `docs/insights-ui/tariffs/tariff-calculator-niche-research.md` with: methodology recap, the exported keyword sheet (or link to it), per-keyword SERP teardown table, Tier 1/2/3 keyword map, KoalaGains surface assignments, internal-link + schema plan, and a 30 / 60 / 90-day execution plan sized so individual follow-up PRs can ship Tier 1 wins without rework. Add the new doc to `docs/insights-ui/tariffs/README.md`.
