...
### Handle failed ETF generation requests

> When an `EtfGenerationRequest` step fails it lands in `failedSteps` and the row shows up in the **Failed** group on `/admin-v1/etf-generation-requests`. Today the only recovery is the per-row **Reload** button (`api/[spaceId]/etfs-v1/generation-requests/[requestId]/reload`), which resets the request to `NotStarted` so the next trigger retries from scratch. The model stores _which_ steps failed (`failedSteps: String[]`) but never _why_ — no error message, code, or stack is persisted (`prisma/schema.prisma` → `EtfGenerationRequest`; failure path in `src/utils/etf-analysis-reports/etf-generation-report-utils.ts`). Failures are detected two ways: an exception during LLM invocation, or the 5-minute stale-step timeout guard. `FINAL_SUMMARY` cascades — if any of the four analysis reports fail it is marked failed too (`etfReportDependencyMap`). Goal of this task: make failed requests diagnosable and recoverable without hand-holding.

- [ ] **Persist a failure reason per failed step.** Add an error field to `EtfGenerationRequest` (e.g. `failureReasons Json?` keyed by report type, or a sibling `EtfGenerationStepError` table) capturing message + a coarse cause (`llm-error` / `timeout` / `dependency-failed` / `validation`). Populate it in the two failure sites in `etf-generation-report-utils.ts` (the catch around LLM invocation and the stale-step timeout branch). Distinguish a cascaded `dependency-failed` from a real failure so the UI doesn't show four identical errors when only one report actually broke.
- [ ] **Surface the reason in the admin UI.** On `app/admin-v1/etf-generation-requests/page.tsx`, show the captured reason per failed step (tooltip on the red status dot or an expandable row) so an admin can tell a transient timeout from a genuine generation error before deciding to reload.
- [ ] **Reload only the failed steps, not the whole request.** Current reload resets the request to `NotStarted` and re-runs everything. Add an option to re-queue _only_ `failedSteps` (preserving `completedSteps`) so a single broken category doesn't force regeneration of the other five. Keep the existing full-reset path as an explicit "regenerate all" action.
- [ ] **Bounded auto-retry for transient failures.** Add a `retryCount` (+ cap) to `EtfGenerationRequest` and auto-reload requests that failed on `timeout` / transient `llm-error` up to the cap before leaving them in the Failed group for manual handling. Do not auto-retry `validation` / hard errors. Surface the retry count in the admin row.
- [ ] **Failed-only view + bulk reload.** The list API already paginates the Failed group (`failedSkip` / `failedTake`); add a filter to show only failed requests and a "Reload all failed" bulk action so a night of failures can be cleared in one click. Cross-check against the `isComplete` definition in **ETFs list page** above — an ETF with a permanently-failed step should not be counted complete.
- [ ] **Alerting / visibility.** Emit a count of currently-failed requests somewhere durable (admin dashboard tile or the off-hours runner's summary) so failures don't silently accumulate between manual admin visits. Tie into the **Claude-Code Sonnet pipeline** runner once it lands.
