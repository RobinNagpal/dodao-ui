...
- [ ] **Baseline ETF page perf** — capture Lighthouse + Vercel Speed Insights numbers (LCP, TTFB, CLS, INP, total page weight, JS bundle size) on representative URLs: `/etfs`, `/etfs/groups/[group]`, `/etfs/countries/[country]`, `/etfs/categories/[category]`, `/etfs/[exchange]/[etf]` (detail), plus one sub-page (`performance-returns`). Record desktop + mobile, cold vs warm. Save baseline to `docs/insights-ui/etf-analysis/perf-baseline.md` so post-fix regressions are obvious.
- [ ] **Remove `dynamic = 'force-dynamic'` from listing + detail ETF pages** — every page under `insights-ui/src/app/etfs/**/page.tsx` and `insights-ui/src/app/etfs/page.tsx` currently sets `force-dynamic`, which opts out of the Next.js Data Cache and forces SSR per request. The fetches already pass `next: { revalidate: TWO_WEEKS_IN_SECONDS, tags: [...] }` and the cache tags are wired into the generation pipeline — `force-dynamic` makes that configuration dead. Switch to ISR (drop `dynamic` or set `dynamic = 'force-static'` where appropriate) and verify on-demand revalidation by tag still triggers rebuilds when an ETF report is regenerated.
- [ ] **Add `generateStaticParams` to ETF detail + sub-pages** — pre-render the `(exchange, etf)` combinations that already have `isComplete = true` reports for `/etfs/[exchange]/[etf]` and the six sub-pages (`performance-returns`, `cost-efficiency-team`, `risk-analysis`, `future-performance-outlook`, `competition`, `holdings`, `financial-data`). Cap initial coverage to the top-N by traffic so build time stays reasonable; let ISR cover the long tail. Coordinate with the `isComplete` filter task above so we don't pre-render in-progress reports.
- [ ] **Stream heavy detail-page sections with Suspense** — `app/etfs/[exchange]/[etf]/page.tsx` waits on a single `full-render` fetch before any HTML ships. Split into above-the-fold (header, badges, key metrics, key facts) and below-the-fold (radar chart, competition chart, analysis sections, holdings, similar ETFs), wrap the heavy below-the-fold blocks in `<Suspense>` with skeleton fallbacks, and have each block fetch its own narrower API slice. Measure LCP improvement against the baseline.
- [ ] **Audit + slim client bundles on ETF pages** — run `ANALYZE=true yarn build` and look for fat client components in the ETF tree (likely candidates: chart libs in `EtfRadarChart`, `EtfCompetitionChartSection`, `EtfChartTabs`). Dynamic-import charts with `ssr: false` where they're not needed for first paint, and confirm leaf chart components are not pulling the full lib when only one chart type is used.
- [ ] **Listing-page payload check** — `EtfGroupsIndex` and per-group/per-country list components: confirm the listings API returns only fields the cards render (name, ticker, exchange, category, key flags); strip large free-text fields (descriptions, full strategy text) from list payloads and lazy-load on detail. Also check pagination/virtualization on long category/group pages.
- [ ] **Image + JSON-LD hygiene** — verify all card images go through `next/image` with explicit `width`/`height` (CLS fix) and that the inline `<script type="application/ld+json">` blocks aren't duplicated by a layout-level injector. Quick win, but easy to miss.
- [ ] **Re-measure + document gains** — after each fix lands, re-run the baseline harness from step 1 and append a delta row to `perf-baseline.md` so the wins are auditable. Stop when LCP < 2.5s on mobile + INP < 200ms on the detail page across the sample URLs.
