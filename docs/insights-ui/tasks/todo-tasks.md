...
- [ ] **Stock indexed pages — apply the crowd-funding / blog SEO treatment to per-section stock pages** (pattern from PRs #1606 and #1603). Cover all seven per-section pages under `app/stocks/[exchange]/[ticker]/` (`business-and-moat`, `competition`, `fair-value`, `financial-statement-analysis`, `future-performance`, `management-team`, `past-performance`) and the main `[ticker]/page.tsx`. Concretely:
  - **Metadata consistency** — title, OG title, twitter title all use the same `"{tickerName} ({symbol}) — {readableSectionName}"` shape; description sourced from each section's summary (truncated ~155 chars via the shared `truncateDescription` helper in `src/util/report-utils.ts`); drop boilerplate keyword lists; KoalaGains logo fallback for OG image when no per-ticker image exists.
  - **JSON-LD** — emit Article (or `FinancialProduct`/`Report` if more appropriate per section) JSON-LD on every per-section page; publisher + author = KoalaGains Organization; include `datePublished` / `dateModified` from the underlying report row; `mainEntityOfPage` set to the canonical URL.
  - **H1 / heading hygiene** — single H1 per page that matches the `<title>`; cascade-demote any in-body headings that collide with the template H1; ensure section labels are humanized (no raw enum values like `business_and_moat`).
  - **Sibling-section internal linking** — bottom-of-page "Other sections for {tickerName}" rail linking to the other per-section pages for the same ticker, mirroring the "Other sections of {projectName}" rail added in #1606.
  - **Per-section sitemap filter** — each per-section sitemap (`business-and-moat-sitemap.xml`, `competition-sitemap.xml`, `fair-value-sitemap.xml`, `financial-statement-analysis-sitemap.xml`, `future-performance-sitemap.xml`, `management-team-sitemap.xml`, `past-performance-sitemap.xml`) only emits URLs whose underlying report row is non-empty and non-failed; pages still resolve by URL but render `robots: noindex` when the report isn't ready, so Google can deindex empty pages and stop wasting crawl budget.
  - **Verification** — after deploy, re-submit affected URLs in Search Console; track the indexed-URL delta per per-section sitemap weekly (reuse the delta report from the sibling indexing task above); confirm no regression on already-indexed pages.
