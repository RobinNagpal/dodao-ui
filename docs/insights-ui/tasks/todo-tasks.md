...
### Caching — build a single canonical understanding

> Caching knowledge is real but scattered across five docs ([cloudfront-deploy-skew.md](../cloudfront-deploy-skew.md), [cloudfront-error-caching.md](../cloudfront-error-caching.md), [stock-page-caching.md](../stock-page-caching.md) (now historical), [aws-deployment.md](../aws-deployment.md), [aws-migration-plan.md](../aws-migration-plan.md)) and seven `*-cache-utils` modules, with no map of how the layers fit together. The point of this task is **understanding first, not new optimization** — produce one authoritative reference so the next person doesn't have to reverse-engineer the topology.

- [ ] **Map every caching layer end-to-end** — trace a request for `/stocks/[exchange]/[ticker]` (and one ETF + one tariff page) through each layer and record what each does, its TTL/key, and how it's invalidated: (1) CloudFront edge (page + API behaviors, the 6-day TTL, `error_caching_min_ttl` for 4xx/5xx — see `deployments/insights-ui/cloudfront.tf`), (2) Vercel/Next.js ISR + Data Cache **and why `force-dynamic` (#1499) makes most of it inert** today, (3) Next.js `revalidateTag` cache-tag graph, (4) the `TickerV1CachedScore` precomputed-score layer, (5) any in-process/module-level memoization. Note that AWS (single-node Lightsail) and Vercel have different cache-coherence behavior.
- [ ] **Inventory the cache-util modules** — for each of `ticker-v1-cache-utils.ts`, `etf-cache-utils.ts`, `etf-scenario-cache-utils.ts`, `stock-scenario-cache-utils.ts`, `tariff-report-cache-utils.ts`, `cloudfront-cache-utils.ts`, `cache-actions.ts`, and `tariff-calculator/cache-tags.ts` (all under `insights-ui/src/utils/`): list the tags it defines, the read paths that attach them, the write/admin paths that invalidate them (incl. `admin-v1/invalidate-cache`), and which CloudFront paths the invalidation also flushes. Flag any read-path utility that calls `revalidate*` while serving a request (a known footgun called out in the ETF perf tasks).
- [ ] **Write `docs/insights-ui/caching-overview.md`** — the single canonical entry point: a layer diagram (edge → CDN cache → app render → tag graph → DB/score cache), a per-layer "what it caches / how it's keyed / how it's invalidated / where it's configured" table, a "which lever to pull when" decision guide (stale page, error pinned at edge, ISR writes too high, deploy skew), and a glossary tying the existing five docs together. Mark `stock-page-caching.md` references as historical where they apply. Update [`docs/insights-ui/README.md`](../README.md) and [`tasks/README.md`](./README.md) to point at it.
- [ ] **Verify the doc against live behavior** — confirm a warm second hit returns `x-cache: Hit from cloudfront` on the detail page and the cached API endpoints; confirm a genuine 404 is not pinned for 6 days; confirm an admin "invalidate cache" actually flushes both the Next tag and the CloudFront path. Capture the commands used so the doc doubles as a runbook. This is a knowledge/documentation task — **do not change caching behavior**; if the investigation surfaces an actual bug, file it as a separate todo rather than fixing it here.
