...
- [ ] **SEO — "Crawled — currently not indexed" on main `/stocks/{exchange}/{ticker}` pages** — ~95 main stock URLs failed the most recent GSC revalidation (out of ~15k flagged + ~682 revalidation failures total). 20-URL teardown + hypothesised root causes in [`../stocks-crawled-not-indexed-pattern-analysis.md`](../stocks-crawled-not-indexed-pattern-analysis.md): dominant failing archetype in the sample is small/micro-cap Healthcare Biopharma (8/20) and non-US tickers (8/20), with a hard ~2.3–2.5k visible-word ceiling on every pre-June-12 report and a per-cron `datePublished` cluster that fingerprints the batch generator. Next steps before writing code: pull full failed-revalidation list and bucket by `(exchange, industryKey, ageInDays, visibleWords, hasMgmtTeam)`; run same teardown on an indexed-control sample to find the actual divider; then decide priority order across (a) backfill Management Team section on pre-June-12 reports, (b) strip trailing whitespace from industry names, (c) stop exposing cron-second precision in `datePublished`, (d) per-ticker hero / Article-schema image, (e) verify `TickerV1.updatedAt` is bumped on every category-report edit so sitemap `lastmod` stays honest, (f) raise internal links to non-US country pages.
