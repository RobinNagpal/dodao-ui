...
### Tariff calculator — SEO research

> Research-only task. Output is a single doc at `docs/insights-ui/tariffs/tariff-calculator-seo-research.md` that downstream PRs (on-page fixes, programmatic-SEO landing pages, internal linking) can pull from. No code changes in this task.

- [ ] **Baseline performance** — pull current Google Search Console impressions / clicks / average position / CTR for `/tariff-calculator` (last 90 days + last 28 days for the trend); record top 25 organic queries the page already ranks for and the top 25 it shows for but ranks 11–30 on (the recoverable band). Note current state in Search Console coverage report ("Indexed", "Crawled — currently not indexed", "Discovered — currently not indexed").
- [ ] **Search intent + keyword landscape** — group target queries into intent buckets (HTS-code lookup, country-specific landed cost, section-specific — 232 / 301 / IEEPA, year-stamped "2026 tariff calculator", de-minimis questions, "China tariff" head terms). For each bucket flag head-term vs long-tail opportunities and whether our current title / description / on-page copy targets it.
- [ ] **Competitor SERP teardown** — analyse the top 10 organic results for the highest-volume tariff/duty calculator queries (SimplyDuty, DutyCalculator, USITC HTS, CBP, Freightos, freight-forwarder calculators, carrier landed-cost tools). For each: what they cover (HTS depth, country selectors, FAQ, methodology, freshness stamps, schema markup), what they do better than us, and the content gaps we can exploit. Keep the writeup vendor-neutral in identifiers — competitor names belong in research prose only, never in code or component names.
- [ ] **On-page audit of `/tariff-calculator`** — score against the keyword buckets above: title + description coverage (currently a single static title — is a per-country / per-section variant warranted?); H1/H2 structure; presence of FAQ block + `FAQPage` JSON-LD; "How it works" / methodology section with a visible "Last updated YYYY-MM-DD" stamp; internal link rails into `/hts-codes`, `/tariff-reports`, country pages; mobile + Core Web Vitals snapshot from PageSpeed Insights.
- [ ] **Indexability + sitemap** — confirm `/tariff-calculator` is in `public/sitemap.xml` (or a child sitemap) with a sensible `lastmod`; verify `robots.txt` allows it; if shareable / prefilled calculator URLs ever ship (`?hts=...&country=...`), document the canonical-tag strategy now so future PRs don't fragment ranking signals.
- [ ] **Programmatic SEO opportunity sizing** — estimate traffic potential for per-country landing variants (`/tariff-calculator/china`, `/mexico`, `/canada`, `/vietnam`, …) and per-HTS-chapter variants with prefilled calculator state. For each candidate set: expected monthly search volume (Search Console + a keyword tool), thin-content risk, what unique per-page content would clear the quality bar (rate snapshot, recent changes, top HS codes from that origin, links to relevant tariff reports), and schema to attach (`WebApplication` + `FAQPage` + `BreadcrumbList`).
- [ ] **AI / answer-engine visibility** — test ChatGPT, Gemini, Perplexity, and Google AI Overviews for "how do I calculate US import duty on …" / "section 301 tariff on \<product\> from China" prompts; note whether the KoalaGains calculator is being cited and what structural changes (dated last-updated stamps, clear methodology page, machine-readable rate tables, llms.txt entry) would improve citation share.
- [ ] **Deliverable** — single research doc at `docs/insights-ui/tariffs/tariff-calculator-seo-research.md` with: baseline metrics table, competitor benchmark matrix, prioritized on-page change list (with effort tags), programmatic-SEO ship list with effort + traffic estimates, and a 30 / 60 / 90-day action plan that hands work off into `[Operational + measurement](#operational--measurement)` and a new follow-up section under [tariffs/README.md](../tariffs/README.md).
