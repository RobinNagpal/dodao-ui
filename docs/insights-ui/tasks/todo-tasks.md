...
### ETF filters — UI polish

> The filter modal + chips were just unified across the listing surfaces (PR #1484, `92606e7b4`). This task captures the next round of UX gaps before we layer on more filter types. Code lives in `insights-ui/src/components/etfs/EtfFiltersButton.tsx`, `insights-ui/src/components/etfs/EtfAppliedFilterChips.tsx`, and `insights-ui/src/utils/etf-filter-utils.ts`; the filtered route is `insights-ui/src/app/etfs-filtered/page.tsx`; the API consumer is `insights-ui/src/app/api/[spaceId]/etfs-v1/listing/route.ts`.

- [ ] **Mobile pass on the filter modal** — `EtfFiltersButton.tsx` renders 15 dropdowns in a `grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5` grid inside a `FullPageModal max-w-6xl`. On a phone the 2-col grid is tall and unscannable; section headings + the MOR period selector also stack awkwardly. Tighten the per-dropdown footprint, switch to a single-column accordion or grouped sections on `< sm`, and verify the Cancel / Apply / Clear-all action row stays visible without the modal scrolling its title off-screen.
- [ ] **Match count before Apply** — modal currently commits filter changes only on Apply (`handleApply` in `EtfFiltersButton.tsx:145`), so users can't tell whether a combination yields 0 ETFs until they navigate. Wire a lightweight count endpoint (or reuse `/api/[spaceId]/etfs-v1/listing` with `pageSize=0`) and show "≈ N ETFs match" that updates as the user changes dropdowns; gate the Apply button on a non-zero count or surface a "no results" hint.
- [ ] **Preserve filter context when drilling into an ETF detail page** — clicking an ETF card on `/etfs-filtered?...` navigates to `/etfs/[exchange]/[symbol]` and the back-button loses the chip state for users coming from a deep-linked share. Add a "Back to filtered results" affordance on the detail page when a `Referer` / saved query exists (or stash the last filtered URL in `sessionStorage` keyed by ETF symbol).
- [ ] **Pre-injected filters from sub-listing routes should be editable in the modal** — `/etfs/categories/[category]`, `/etfs/groups/[group]`, `/etfs/asset-classes/[assetClass]`, `/etfs/providers/[provider]`, `/etfs/countries/[country]` each auto-inject a filter into searchParams before render. Today the only way to remove that filter is via the chip; the dropdown in the modal still reflects the auto-injected value but switching pages on apply is non-obvious. Either route through `/etfs-filtered` consistently once the user opens the modal, or make the auto-injected filter visually distinguishable in the modal with a "remove" affordance.
- [ ] **`isComplete` filter wiring** — the `Etf.isComplete` field + admin-only toggle described in the ETFs "Top priorities" section needs to land as a first-class filter in `etf-filter-utils.ts` (param key + applied-filter chip), default-on for public users, hidden behind the admin "Include incomplete ETFs" toggle. Make sure the chip representation reads naturally ("Showing complete ETFs only" rather than `isComplete = true`). Depends on the top-priority `isComplete` task; this is the UI surface of it.
- [ ] **Sort control alongside filters** — `EtfListingGrid` currently has no user-facing sort; the API returns a default order. Add a sort dropdown (AUM desc, expense ratio asc, name asc, etc.) either next to the Filters button in `EtfPageLayout.tsx` or as a section inside the modal, URL-synced via a new `sort=` param so it survives shareable links.
- [ ] **Search visibility + chip** — the symbol/name search query param applies through the API but isn't surfaced as a chip in `EtfAppliedFilterChips.tsx`. Add it as a clearable chip ("Search: foo ×") so users know why their result set is narrowed when they arrive via a saved link.
- [ ] **"Reset filters" UX is split between the modal and the chip row** — modal has an underlined "Clear all filters" link (`EtfFiltersButton.tsx:321`) that only clears local state; the chip row has its own "Clear all" that clears the URL. Unify the wording, place, and behavior so a user sees one consistent control regardless of where they're looking.
- [ ] **Telemetry on filter use** — once the polish lands, log filter-open / filter-apply / clear-all events with the active filter set so we can see which dropdowns are actually used and prune dead weight. Feeds into a follow-up "trim the filter set" decision.
- [ ] Open: should the modal also expose category / group / country / provider / asset-class as dropdowns (currently they're only set by navigating to the corresponding sub-listing route), or keep those scoped to the route-driven entry points? Same question for `isComplete`-style boolean filters — modal section vs admin-only toggle outside the modal.
