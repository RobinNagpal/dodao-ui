...
- [ ] **SEO — "Crawled — currently not indexed" on main `/stocks/{exchange}/{ticker}` pages** — ~95 main stock URLs failed the most recent GSC revalidation (out of ~15k flagged + ~682 revalidation failures total). 30-URL teardown + hypothesised root causes in [`../stocks-crawled-not-indexed-pattern-analysis.md`](../stocks-crawled-not-indexed-pattern-analysis.md): dominant failing archetype is small/micro-cap Healthcare Biopharma (10/30), non-US tickers (10/30, with every Korean URL = Biopharma microcap), and a hard ~2.3–2.5k visible-word ceiling on every pre-June-12 report (10/16 older URLs land in that band). The per-cron `datePublished` fingerprint is sharp — three ASX URLs share `datePublished` to the millisecond and 10 NASDAQ/NYSE URLs share a 111 ms window on 2026-06-12T20:43:20. Next steps before writing code: pull full failed-revalidation list and bucket by `(exchange, industryKey, ageInDays, visibleWords, hasMgmtTeam)`; run same teardown on an indexed-control sample to find the actual divider; then decide priority order across (a) backfill Management Team section on pre-June-12 reports, (b) strip trailing whitespace from industry names, (c) stop exposing cron-second precision in `datePublished`, (d) per-ticker hero / Article-schema image, (e) verify `TickerV1.updatedAt` is bumped on every category-report edit so sitemap `lastmod` stays honest, (f) raise internal links to non-US country pages.
