...
- [ ] **Expand admin "Edit Stock Details" modal** — `EditStockDetailsModal.tsx` (invoked from `StockActionsAdminPanel.tsx`) currently only edits forwarding fields (`movedExchange`, `movedSymbol`, `isDeleted`). Broaden it to cover the rest of the admin-editable `TickerV1` fields: `name`, `industryKey` + `subIndustryKey` (must be edited as a pair to respect the composite FK), `websiteUrl`, `summary`, `aboutReport`, `metaDescription`, and `stockAnalyzeUrl`. Group the form into Identity / Classification / Content / Routing & Status sections so the modal stays scannable on mobile. Extend `UpdateStockAnalyzeUrlRequest` and the `PUT /api/[spaceId]/tickers-v1/exchange/[exchange]/[ticker]` handler to accept and validate the new fields (industry ↔ sub-industry pairing via `TickerV1SubIndustry`, URL format checks, length caps, trimmed-string null coercion); reuse the existing `revalidateAllTickerTags` cache bust on save and audit who edited what via `updatedBy`. Add a follow-up admin diff/log view if multiple admins start editing the same tickers.
