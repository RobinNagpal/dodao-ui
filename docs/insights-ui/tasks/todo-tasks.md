...
### ETF listing pages — fix filters

- [ ] **Fix filters across every ETF listing surface** — the filter modal (`EtfFiltersButton.tsx` → `EtfFilterModalContent` → `applySelectedEtfFiltersToParams` in `src/utils/etf-filter-utils.ts`) and the applied-filter chips (`EtfAppliedFilterChips.tsx`) currently hand off to `/etfs-filtered` via a blanket `pathname.replace('/etfs', '/etfs-filtered')`, which breaks sub-listings (`/etfs/groups/[group]`, `/etfs/categories/[category]`, `/etfs/countries/[country]`, `/etfs/asset-classes/[assetClass]`, `/etfs/providers/[provider]`) because no `/etfs-filtered/...` variants exist for them. Audit each listing surface and: (a) confirm the filter button is actually rendered and wired up, (b) decide whether to route the filtered view inline on the same page (preserve the category/country/group/asset-class/provider scope when filters apply) or to add the missing `/etfs-filtered/...` variants, (c) make the chip "remove" / "clear all" reversal match the chosen routing scheme so the user lands back on the originally-scoped page (not the global `/etfs`), (d) verify every advertised filter — AI category scores + total score, AUM, expense ratio, P/E, dividend (TTM / yield / payout frequency / years), holdings, volume, beta, asset class, issuer, and the advanced Mor period-based risk filters — actually narrows results on the resulting page and that the active filter chips render the right labels, (e) handle the empty-result state cleanly. Cross-check against the `isComplete` filter once it lands so both filters compose correctly.
