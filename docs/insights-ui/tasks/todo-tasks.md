...
- [ ] **Add `categoryInstructions` to the ETF factor JSON** — each factor in `insights-ui/src/etf-analysis-data/etf-analysis-factors-*.json` today only carries `groupInstructions` keyed by the 8 high-level groups (`broad-equity`, `sector-thematic-equity`, `leveraged-inverse`, `commodities-and-digital-assets`, `derivative-income`, `allocation-target-date`, `fixed-income-investment-grade`, `fixed-income-credit-and-income`). Many Morningstar categories inside a group need their own framing — broad EM (`Diversified Emerging Mkts`) is a regional-diversification sleeve, not a thematic bet; long-duration government swings very differently from intermediate core; bank loans behave nothing like HY corporates; managed-futures and market-neutral share no decision frame with covered-call funds. Add an optional `categoryInstructions` map keyed by the Morningstar category name (the same `name` values listed in `etf-analysis-categories.json` → `categories[]`) on each factor.
- [ ] Plumb `factorAnalysisCategoryInstructions` through the prompt loaders + render it in each prompt template (`docs/insights-ui/etf-prompts/past-returns.md`, `cost-efficiency-team.md`, `risk-analysis.md`, `future-performance-outlook.md`) alongside the existing `factorAnalysisGroupInstructions` block. Document precedence in each prompt: category > group > generic factor description. Use the Morningstar category from `overviewCategory` to pick the right block at render time.
- [ ] Populate sparingly — only fill `categoryInstructions` for categories whose decision frame materially diverges from the rest of its group; leave the rest empty so group instructions remain authoritative. First targets: broad EM equity (`Diversified Emerging Mkts`) inside `sector-thematic-equity`; bank loans + securitized credit (`Bank Loan`, `Securitized Bond - Focused`) inside `fixed-income-credit-and-income`; long-duration government (`Long Government`, `Long-Term Bond`) inside `fixed-income-investment-grade`; managed-futures / market-neutral / long-short (`Systematic Trend`, `Equity Market Neutral`, `Long-Short Equity`) inside `derivative-income`.
- [ ] Re-run the prompt-finalization loop (`docs/insights-ui/etf-prompts/prompt-finalization-approach.md`) on a sampled set spanning each newly-instructed category to confirm outputs improve for those funds without regressing the rest of the group.
- [ ] Open: per-factor `categoryInstructions` (mirrors today's `groupInstructions` shape and lets each factor pick its own override) vs. a single per-category block injected once near the top of the prompt (keeps the factor JSON scannable but loses per-factor granularity). Decide before bulk-filling. Also: does this work get absorbed by the "Carve broad EM out of `sector-thematic-equity`" / "Re-introduce a floating-rate bond group" follow-ups below — i.e. is the right fix to add a new *group* in some cases, and reserve `categoryInstructions` only for the long tail?
