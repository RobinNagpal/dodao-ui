...
- [ ] **SEO — "Crawled — currently not indexed" on main `/stocks/{exchange}/{ticker}` pages** — ~95 main stock URLs failed the most recent GSC revalidation (out of ~15k flagged + ~682 revalidation failures total). 48-URL teardown + hypothesised root causes in [`../stocks-crawled-not-indexed-pattern-analysis.md`](../stocks-crawled-not-indexed-pattern-analysis.md): three distinct failing slices — (a) data-poor small/micro-cap Healthcare Biopharma + non-US microcaps clustered at ~2.3–2.6k visible words (Healthcare Biopharma 14/48 [29%], non-US 19/48 [40%], Korean URLs now 6/8 Biopharma not 100% after KOSDAQ/420570 + KOSDAQ/001000 broke the cluster), (b) non-US tickers with adequate content but limited country-page link equity (ASX 10/48 across 5 industries including mid-cap SGH, Korean non-Biopharma, BSE/539872), and (c) US large/mid-cap names in competitive query landscapes (BURL, NVR, CGEN) where pages produce 3–8k words but compete head-on with Yahoo / MarketWatch / Seeking Alpha / biotech-specialist publications. **NASDAQ/CGEN is the key counter-example**: 7,674 words *with* Management Team, re-modified 2026-05-02, still failing on 2026-06-17 — proves backfilling Mgmt Team alone is not sufficient (there's a multi-month "time-to-recover-from-demotion" lag on top of the content problem). The cron-batch `datePublished` fingerprint is sharp (ASX 14:31 pulse now 6 URLs ms-identical; ASX 12:46 pulse 4 URLs ms-identical; June-12 20:43:20 pulse 13 URLs in 135 ms = 27% of sample) but is still a *plausible weak signal* rather than a confirmed Google ranking factor. The HTML-weight finding is also weaker than initially framed — 92.5% of every page is the Next.js RSC streaming payload (analysis text shipped twice for hydration), 0 bytes of inline CSS, charts not inlined. Next steps before writing code: pull full failed-revalidation list and bucket by `(exchange, industryKey, ageInDays, visibleWords, hasMgmtTeam, marketCapBand)`; run same teardown on an indexed-control sample to find the actual divider; then decide priority order across (a) backfill Management Team section on pre-June-12 reports (necessary but not sufficient — pair with link-equity work), (b) strip trailing whitespace from industry names ("Healthcare: Technology & Equipment " hits 5 URLs in the sample alone), (c) per-ticker hero / Article-schema image, (d) verify `TickerV1.updatedAt` is bumped on every category-report edit so sitemap `lastmod` stays honest, (e) raise internal links to non-US country pages (ASX, Korea, India), (f) (low priority) round `datePublished` to date-only.
