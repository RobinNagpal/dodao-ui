...
- **[add-new-report-type.md](add-new-report-type.md)** — End-to-end runbook for adding a brand-new stock **report type** (a new `ReportType` alongside Financial Analysis, Competition, Management Team, etc.): the full touch-point checklist across the enum, generation-request model, API route, step/prompt/save plumbing, and the detail sub-page, with `MANAGEMENT_TEAM` as the worked example.
