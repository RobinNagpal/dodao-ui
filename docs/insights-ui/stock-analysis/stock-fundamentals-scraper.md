...
`STOCK_ANALYZER_LAMBDA_URL` is no longer read by anything; the scraper reads
`NEXT_PUBLIC_STOCK_ANALYZE_BASE_URL` instead. (`ETF_ANALYZER_LAMBDA_URL`,
`ETF_MORN_LAMBDA_URL` and `SCREENER_API_URL` are unrelated and still in use.)
...
| `index.ts` | Section registry (URL + parser + usability test) and `scrapeStockAnalyzerSection()`. |
