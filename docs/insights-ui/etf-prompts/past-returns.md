...
- **Passive index tracker — inside tolerance = Pass.** If the fund tracks its stated index within `0.5 pp` for equities / core bonds or `1.0 pp` for high-yield, muni, sector, or thematic funds across most periods, it has done its job → **Pass** on return factors, even when absolute returns are modest. A gap that sits *inside* the tolerance band — for example a high-yield fund trailing its index by `0.93 pp` over 10 years — is a Pass, not a Fail. Low absolute returns reflect the asset class, not fund failure. *Slightly beating the index also counts as Pass.*
- **Active fund**: **Pass** when the manager has beaten the group-appropriate benchmark named in `factorAnalysisGroupInstructions` (or peer median, whichever the factor targets) net of fees over the horizon the factor covers.
- **Mandate-specific funds** (daily-leveraged / inverse, commodities, defined outcome, covered-call / derivative income, managed futures, long-short, etc.): judge against the stated mandate. Do not force a broad-equity benchmark. A covered-call / derivative-income fund (e.g. `JEPI`) that gave up equity upside for income and downside protection has *delivered on its mandate* → **Pass** on `short_term_returns`, `mandate_return_alignment`, and `yield_and_distribution_history` even when total return trails the S&P 500. Trailing the broad-equity benchmark *is* the strategy.
...
- **Technicals only apply to three groups.** `technical_trend_position` is only mapped to `sector-thematic-equity`, `leveraged-inverse`, and `commodities-and-digital-assets` — the three groups where entry timing is genuinely decision-useful for retail. It will not appear in the `factorAnalysisArray` for broad-equity / allocation / bond / derivative-income funds, so do not invent a technicals section for those groups.
