...
## The four per-category analysis prompts are now read directly from the app

The Past Returns / Cost & Team / Risk / Future Outlook prompts are **no longer copied into the DB** — the code reads them at render time from version-controlled markdown in the app at **`insights-ui/etf-prompts/`** (`resolveEtfPromptTemplate` in `etf-prompt-template-utils.ts`). Edit those four files to change the live prompts; no DB sync step. They live inside the app (not here under `docs/`) because `docs/` does not ship with the Next deployment, while app-root dirs like `etf-prompts/`, `schemas/`, and `blogs/` do.
