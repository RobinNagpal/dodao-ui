...
- **[etf-best-picks/](etf-best-picks/)** — Independent curated shortlist of the 2 best ETFs under each group on `/etfs/groups`. Picks are not derived from KoalaGains' internal scoring — they are opinions based on the underlying index, forward demand, and the ETF wrapper vs its competitors. See [etf-best-picks/README.md](etf-best-picks/README.md).
