...
- **[cloudfront-deploy-skew.md](cloudfront-deploy-skew.md)** — Why `/stocks/*` on koalagains.com sometimes shows "Something went wrong" after a Vercel deploy (CloudFront caches HTML from build N while the live JS comes from build N+1) and the deploy-safe fix: `deploymentId: process.env.VERCEL_DEPLOYMENT_ID` in `next.config.js`, Vercel Skew Protection, and lowering CloudFront `min_ttl` so origin `Cache-Control` is respected.
