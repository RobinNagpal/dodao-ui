...
- **[etf-best-picks.md](etf-best-picks.md)** — Independent curated shortlist of the 2 best ETFs under each of the 8 groups on `/etfs/groups`. Each pick evaluated in a single table on: expense ratio vs same-index competitors, tracking difference across 1y/3y/5y/10y/15y, AUM, portfolio turnover, dividend distribution, max drawdown + recovery vs index, forward-holdings outlook, securities-lending revenue, and bid-ask spread.
- **[etf-best-picks-justifications.md](etf-best-picks-justifications.md)** — Reasoning behind each pick in `etf-best-picks.md`: why this ETF beats its same-theme competitors, scenario-mapping for thematic picks, decision rules and fallbacks.
