...
- **Invalidation is wired end-to-end:** every save-flow `revalidate*` helper purges the affected page URL **and** the matching API URL in lockstep (`ticker-v1-cache-utils.ts`, `etf-cache-utils.ts`). No API endpoint is cached without a corresponding purge path.
- **Net assessment:** API caching is correct and internally consistent (Terraform ↔ `CACHED_PATH_PREFIXES` ↔ per-save invalidation all agree). The problems found are **stale documentation and code comments**, not a functional caching gap — see §6.

---

## 2. What is cached (verified against `cloudfront.tf`)
...
| CloudFront path pattern | Backs the page |
|---|---|
| `/api/koala_gains/tickers-v1/exchange/*/*/full-render` | `/stocks/{e}/{t}` (main aggregate) |
...
| `/api/koala_gains/tickers-v1/exchange/*/*/competition` | `/stocks/{e}/{t}/competition` |
| `/api/koala_gains/tickers-v1/exchange/*/*/management-team` | `/stocks/{e}/{t}/management-team` |
...
| `/api/koala_gains/etfs-v1/exchange/*/*/analysis` | `/etfs/{e}/{t}/{cost-efficiency-team,risk-analysis,future-performance-outlook}` |
...
**Not cached** (deliberately pass through the `Managed-CachingDisabled` default):
mutating routes (`PUT /exchange/{e}/{t}`, `POST /save-report-callback`, etc.), admin GETs
under the same prefixes (`/generation-requests`, `/missing-reports`, `/listing`, …), the ETF
`competition` subpage's base `/exchange/{e}/{t}` route, and everything under other API trees
(scenarios, tariff-calculator, hts-codes, auth, portfolio-managers, daily-movers).
...
```
Browser ──► CloudFront ──(page miss on /stocks/{e}/{t})──► Lightsail origin (SSR, force-dynamic)
                                                              │
                                    SSR self-fetches its data │  fetch(`${getBaseUrl()}/api/koala_gains/tickers-v1/exchange/{e}/{t}/full-render`)
                                                              ▼
...
behaviors existed, those data fetches passed straight through to the origin. Now a page miss on a
popular ticker/ETF resolves its data from the already-warm **API** cache, so the origin does DB/S3
...
The **implementation is correct and self-consistent**; the drift is in docs/comments written
before the Lightsail migration and the ETF-API-caching work landed. Recorded here so the next
reader trusts the code over the older prose:
...
None of these change runtime behavior — they are correctness-of-documentation fixes.
...
```bash
# Is a cached stocks API endpoint being served from the edge?
curl -sI 'https://koalagains.com/api/koala_gains/tickers-v1/exchange/NYSE/RTX/full-render' \
  | grep -iE 'HTTP|x-cache|age|content-type'
# Expect: HTTP/2 200 ; x-cache: Hit from cloudfront (after warm) ; age: <secs> ; application/json

# Same for an ETF endpoint
