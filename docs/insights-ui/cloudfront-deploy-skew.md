...
8. [The `flush-cloudfront-cache` GitHub workflow](#the-flush-cloudfront-cache-github-workflow)
9. [The deploy-skew bug, in detail](#the-deploy-skew-bug-in-detail)
10. [The deploy-skew fix](#the-deploy-skew-fix)
11. [Where CloudFront is slow today](#where-cloudfront-is-slow-today)
12. [Cache constraints](#cache-constraints)
13. [Next options](#next-options)
14. [Cache helper plumbing — what each helper does today](#cache-helper-plumbing--what-each-helper-does-today)
15. [Operational guide](#operational-guide)
16. [Where the pieces live (file paths)](#where-the-pieces-live-file-paths)
...
### Phase 4 — Targeted CloudFront invalidation on save + tariff cache + manual flush workflow

After Phase 3, content freshness on `/stocks/*` and `/etfs/*` was bounded by the 6-day TTL — when an admin saved an updated report or edited stock details, the upstream Vercel `revalidateTag()` call was inert (the routes are `force-dynamic`, nothing to invalidate), and CloudFront kept serving the old HTML at the edge until natural TTL expiry. Users could see stale content for nearly a week after a save.

Phase 4 closes that loop by invalidating CloudFront at the same moment as the upstream tag. Three coordinated changes:

1. **A new helper `invalidateCloudFrontPaths(paths)` (`insights-ui/src/utils/cloudfront-cache-utils.ts`).** Calls AWS CloudFront's `CreateInvalidation` API with the given path(s). Lives behind `waitUntil()` so it runs in the background without blocking the response. Silently no-ops when `CLOUDFRONT_DISTRIBUTION_ID` is unset (preview / local dev). Filters out paths that aren't behind a CloudFront cache behavior so we don't waste the monthly free quota.

2. **Every `revalidate*` helper for a CloudFront-cached path now also calls the helper.** `ticker-v1-cache-utils.ts`, `etf-cache-utils.ts`, and `tariff-report-cache-utils.ts` were retrofitted so that each save-event flow (admin "Invalidate cache" button, LLM report save callback, admin edit save) fires both `revalidateTag()` (Vercel-side, inert under `force-dynamic`) and `invalidateCloudFrontPaths()` (real CloudFront purge) in lockstep. The Vercel-side call remains for two reasons: (a) it's free, and (b) it future-proofs against any route accidentally losing `force-dynamic`.

3. **The cacheable URL surface expanded to include tariff report pages.** `/industry-tariff-report/*` and `/tariff-reports*` got their own `ordered_cache_behavior` blocks in `cloudfront.tf`, using the same `koalagains_one_week` cache policy. The `tariff-report-cache-utils.ts` helper was split into `revalidateTariffReportIndustry` and `revalidateTariffReportChapter` — the same `tariffReportTag(id)` function had been doing double duty for two different URL topologies, so the caller now has to disambiguate which CloudFront URL to purge. `/tariff-calculator/*` and `/hts-codes/*` stay outside CloudFront — both are `force-dynamic` and would break under HTML caching.

4. **`flush-cloudfront-cache.yml` GitHub workflow.** A `workflow_dispatch` workflow with three boolean inputs (`flush_stocks`, `flush_etfs`, `flush_tariffs`). Tick any combination of boxes and the workflow combines them into a single `aws cloudfront create-invalidation` call. Used for emergencies, bulk operations, or after a large batch update.

IAM setup that enables all of this:
- `insights-ui-project-policy` IAM policy in `dodao-api-v2-deployment/iam.tf` grants `CreateInvalidation` / `GetInvalidation` / `ListInvalidations` scoped to the koalagains distribution only.
- The policy is attached to IAM user `insights-ui-vercel-project` (formerly `ai-insights-agent`), whose access key is configured as the Vercel runtime's `AWS_ACCESS_KEY_ID`. The Vercel runtime calls CloudFront with that identity.
- For the GitHub Actions workflow, the existing `KOALA_AWS_*` secrets are reused — the policy needs to be attached to whichever IAM user those secrets correspond to (the workflow's first run prints the identity via `aws sts get-caller-identity` to surface this).
...
This matches **everything except** the four cached prefixes. With `Managed-CachingDisabled`, CloudFront forwards every request to Vercel without caching. Used for:
...
- `/tariff-calculator/*` and `/hts-codes/*` (both `force-dynamic`)
...
#### `/industry-tariff-report/*` ordered behavior

Identical configuration to `/stocks/*`. Covers the cover page, all per-industry subpages (`understand-industry`, `industry-areas`, `tariff-updates`, `final-conclusion`, `evaluate-industry-areas`, `all-countries-tariff-updates`, `generate-all`), and chapter routes under `/industry-tariff-report/chapters/<slug>` + their subpages.
...
Path-pattern invalidations (e.g., `/stocks/*`) count as **1 path each**. Per-save invalidations (Phase 4) fire one path per save flow; bulk helpers use a single wildcard to stay at 1 path even when 8+ URLs need purging. At our scale this is well under the 1,000-path/month free quota — invalidation cost is effectively zero. We do still avoid per-deploy invalidations because they bypass Skew Protection's safety margin, not because of cost.
...
## Cache invalidation today

CloudFront has two invalidation paths, both routed through AWS's `CreateInvalidation` API:

1. **Automatic, per-save targeted invalidation** — fires from inside Vercel functions when something user-visible changes. Wired into every `revalidate*` helper for a CloudFront-cached path.
2. **Manual bulk flush** — fired from the [`flush-cloudfront-cache` GitHub workflow](#the-flush-cloudfront-cache-github-workflow) when an operator needs to wipe a whole prefix at once.

Both share the same IAM policy (`insights-ui-project-policy`) scoped to the koalagains distribution only.

### Automatic per-save invalidation

The wiring lives in two places:

**`insights-ui/src/utils/cloudfront-cache-utils.ts` — the helper itself.**
...
export function invalidateCloudFrontPaths(paths: ReadonlyArray<string>): void {
  ...
  // 4. Errors are logged but never thrown — a failed CloudFront call must
  //    never break the upstream save flow.
}
```
...
- `utils/ticker-v1-cache-utils.ts` portfolio-manager / home-page / daily-mover helpers — their URLs (`/portfolio-managers/*`, `/`, `/daily-top-movers/*`) live under the default `Managed-CachingDisabled` behavior, so there's no edge cache to purge.
- `utils/tariff-calculator/cache-tags.ts`, `utils/tariff-cross-links/hts-chapter-ref.ts` — `/tariff-calculator/*` and `/hts-codes/*` are `force-dynamic` and not behind a cache behavior.
- `utils/stock-scenario-cache-utils.ts`, `utils/etf-scenario-cache-utils.ts` — scenario URLs are not cached at CloudFront.

The "intentionally not wired" group exists because firing `invalidateCloudFrontPaths()` for a path that isn't cached would just consume the monthly free quota for no benefit. The helper's path filter is the safety net (it would silently drop the call anyway), but skipping the call entirely is cleaner.

### Why `revalidateTag()` still runs alongside CloudFront invalidation

Every `revalidate*` helper calls **both** `revalidateTag()` (Vercel-side) and `invalidateCloudFrontPaths()` (CloudFront-side). Under `force-dynamic`, the `revalidateTag()` call is inert today — there's nothing in Vercel's ISR or Data Cache to invalidate.

We keep the call because:
- It's free (in-process, no network).
- It future-proofs against any route accidentally losing `force-dynamic`. If a page goes back to ISR or partial pre-rendering, the existing tag plumbing automatically resumes working.
- It's symmetric with the helper's name (`revalidateXTag` should obviously call `revalidateTag`).

### Cost

CloudFront invalidations:
- First 1,000 paths/month free. $0.005/path beyond that.
- A single path can be exact (`/stocks/NYSE/RTX`) or a wildcard (`/stocks/NYSE/RTX*`). Either way it counts as **one billable path**.
- Bulk helpers (`revalidateAllTickerTags`, `revalidateAllEtfTags`) use a single wildcard to keep cost at 1 path even when invalidating 8+ URLs.

At our scale (admin "Invalidate cache" actions, LLM report saves, and the cron-driven regen cycles for stocks/etfs) we're well below 1,000 paths/month even before counting the manual workflow runs.

### Environment requirements (Vercel)

For the Vercel runtime to actually call CloudFront, set these on the insights-ui project:

| Env var | Value | Notes |
|---|---|---|
| `CLOUDFRONT_DISTRIBUTION_ID` | `EZI5H8FKNE9R1` | When unset, the helper is a silent no-op. Safe default for preview / local dev. |
| `AWS_ACCESS_KEY_ID` | (access key of `insights-ui-vercel-project` IAM user) | Already configured for other AWS calls. |
| `AWS_SECRET_ACCESS_KEY` | (matching secret) | Already configured. |
| `AWS_REGION` | `us-east-1` | CloudFront's control plane is global but its SDK endpoint lives in us-east-1. |

The `insights-ui-project-policy` IAM policy (defined in `dodao-api-v2-deployment/iam.tf`) is attached to that user and grants `cloudfront:CreateInvalidation` / `GetInvalidation` / `ListInvalidations` scoped to the koalagains distribution only.

---

## The `flush-cloudfront-cache` GitHub workflow

Lives in `.github/workflows/flush-cloudfront-cache.yml`. Manually-triggered (`workflow_dispatch`) workflow that purges any combination of the three cacheable URL families.
...
The workflow fails fast if no boxes are ticked. All ticked groups are combined into a single `aws cloudfront create-invalidation` call.

### When to use it

- After a bulk data update that touches many pages at once (re-running a cron, batch-editing a country's industries, etc.) and you want users to see the new content immediately instead of within 6 days.
- After an emergency correctness fix that needs to reach users immediately.
- To pull a published page that should not be visible (legal, security, etc.).

### When NOT to use it

- After a normal deploy. Skew Protection + 6-day TTL handles deploys correctly; flushing here just creates a thundering-herd of cold renders.
- To "freshen" data on a single page. The per-save invalidation already handles that. Use the workflow only for bulk cases.
- Because someone says "the cache feels stale." Find out what specific URL is stale first — it's usually a different problem (e.g., the upstream data didn't actually update).

### The temporary debug step

The workflow currently includes an `aws sts get-caller-identity` step at the top. Its only purpose is to log the ARN of the IAM identity behind the `KOALA_AWS_*` secrets — once we've verified which user that is and attached `insights-ui-project-policy` to it, the debug step can be removed in a follow-up PR.

Until the policy is attached, the workflow's `create-invalidation` step will fail with `AccessDenied`. The debug step's output is what unblocks the attachment.
...
5. **Targeted per-save invalidations are routine; per-deploy invalidations are not.** Each save invalidates a single URL (or a single wildcard) — well below the 1,000-path/month free quota at our scale. Per-deploy CloudFront invalidations remain off the table: they bypass Skew Protection's safety margin and create a thundering-herd of cold renders.
...
## Cache helper plumbing — what each helper does today

The cache helpers below were originally pure ISR plumbing — calls to `revalidateTag()` that flushed Vercel's HTML and Data Caches for the affected route. After Phase 3 (`force-dynamic` everywhere), the `revalidateTag()` half went inert (nothing in Vercel's cache to invalidate). After Phase 4, the same helpers got a second responsibility: calling `invalidateCloudFrontPaths()` for the corresponding URL.

The result is a partially-live, partially-inert state. The helpers fire on the same triggers as before; the Vercel half is a no-op; the CloudFront half is real. Don't remove these unless you also remove their callers — the CloudFront purge would silently stop happening.

### The stock-page tag map (still wired, both halves)

Each per-ticker page used to subscribe to exactly one cache tag. Under `force-dynamic`, the `revalidateTag()` half is inert. Each helper **also** invalidates the matching CloudFront URL — that half is live.

| Route | Tag (Vercel-side, inert) | CloudFront URL invalidated |
|---|---|---|
| `/stocks/[exchange]/[ticker]` (main) | `tickerAndExchangeTag(t,e)` | `/stocks/{e}/{t}` |
| `/stocks/[exchange]/[ticker]/business-and-moat` | `tickerCategoryReportTag(t,e,BusinessAndMoat)` | `/stocks/{e}/{t}/business-and-moat` |
| `/stocks/[exchange]/[ticker]/financial-statement-analysis` | `tickerCategoryReportTag(t,e,FinancialStatementAnalysis)` | `/stocks/{e}/{t}/financial-statement-analysis` |
| `/stocks/[exchange]/[ticker]/past-performance` | `tickerCategoryReportTag(t,e,PastPerformance)` | `/stocks/{e}/{t}/past-performance` |
| `/stocks/[exchange]/[ticker]/future-performance` | `tickerCategoryReportTag(t,e,FutureGrowth)` | `/stocks/{e}/{t}/future-performance` |
| `/stocks/[exchange]/[ticker]/fair-value` | `tickerCategoryReportTag(t,e,FairValue)` | `/stocks/{e}/{t}/fair-value` |
| `/stocks/[exchange]/[ticker]/competition` | `tickerCompetitionTag(t,e)` | `/stocks/{e}/{t}/competition` |
| `/stocks/[exchange]/[ticker]/management-team` | `tickerManagementTeamTag(t,e)` | `/stocks/{e}/{t}/management-team` |

`revalidateAllTickerTags(t,e)` calls `revalidateTag` 8 times (inert) and invalidates `/stocks/{e}/{t}*` once (single wildcard — 1 billable path). Still wired up in:
- `StockActions.tsx` (admin "Invalidate cache" button) — now actually purges CloudFront
...
Vercel logs may still show "Cache invalidated for ticker_..." messages — those refer to the inert `revalidateTag` half. Look for `[cloudfront] Invalidated /stocks/...` in the same log stream for the live half.

### The ETF tag (still wired, both halves)
...
In Phase 1, one main-ETF-page rebuild = 1 HTML + 8 Data Cache writes = **9 cache writes**. One `revalidateEtfAndExchangeTag` call invalidated all 4 ETF surfaces. After Phase 3, the `revalidateTag` half went inert. After Phase 4, each `revalidate*` helper also calls `invalidateCloudFrontPaths()` so the matching `/etfs/{e}/{s}/...` URL is purged from CloudFront. `revalidateAllEtfTags(s,e)` uses a single `/etfs/{e}/{s}*` wildcard for cost efficiency.

### The tariff-report tag (split in Phase 4)

`utils/tariff-report-cache-utils.ts` previously exposed a single `revalidateTariffReport(id)` helper. Because the same `tariffReportTag(id)` function is used by both `/industry-tariff-report/<industryId>` (legacy) and `/industry-tariff-report/chapters/<chapterSlug>` pages, the helper couldn't determine which CloudFront URL to invalidate. Phase 4 split it into two:
...
The only caller (`scripts/industry-tariff-reports/tariff-report-repository.ts`) now passes the chapter slug to the Chapter helper and the legacy `oldUrl` to the Industry helper.

`revalidateTariffReportsListing()` invalidates `/tariff-reports*` (a single billable wildcard).
...
### Cache helper modules — what each does today
...
| `utils/stock-scenario-cache-utils.ts` | Inert | Not wired — same reason |
| `utils/tariff-calculator/cache-tags.ts` | Inert | Not wired — `/tariff-calculator/*` is `force-dynamic` |
| `utils/tariff-cross-links/hts-chapter-ref.ts` | Inert | Not wired — `/hts-codes/*` is `force-dynamic` |
| `utils/ticker-full-render-utils.ts` | Inert | Not wired (utility, not a route invalidator) |

Cleanup of the inert `revalidateTag` half is a follow-up PR — it can be done independently of the CloudFront half. The CloudFront half should stay until those URL families are removed from `cloudfront.tf`.
...
### When to use CloudFront invalidations

For routine save flows: **don't manually invalidate** — the per-save flow in section 7 already handles it. Save an admin change, the helper purges the affected URL automatically.

For bulk operations or emergencies: use the [`flush-cloudfront-cache` GitHub workflow](#the-flush-cloudfront-cache-github-workflow) (section 8). Tick the relevant cache groups and run it.

For one-off paths the workflow doesn't cover, the AWS CLI still works:

```bash
aws cloudfront create-invalidation \
  --distribution-id EZI5H8FKNE9R1 \
  --paths "/stocks/NYSE/RTX*"
```

Cost: 1 path from your monthly 1,000 free quota. Effectively free.

**Reasons to invalidate manually:**
- The per-save invalidation didn't fire (bug, env var missing, IAM unset) and an admin needs a fix immediately.
- Shipping an emergency correctness fix that needs to reach users without the natural TTL wait.
...
- To "freshen" data on a single page that an admin just saved — the save flow already does this. If users still see stale content, the bug is upstream (env var unset, IAM denied, etc.), not in the cache layer.
...
- `.github/workflows/flush-cloudfront-cache.yml` — the manual bulk-flush workflow (see section 8).

### Vercel-side (Next.js)

- `insights-ui/src/app/**/page.tsx` (37+ routes) — `export const dynamic = 'force-dynamic';`
- `insights-ui/src/utils/cloudfront-cache-utils.ts` — `invalidateCloudFrontPaths()` helper. Owns the path filter that defines which URL prefixes get forwarded to AWS.
- `insights-ui/src/utils/ticker-v1-cache-utils.ts`, `etf-cache-utils.ts`, `tariff-report-cache-utils.ts` — every `revalidate*` helper for a CloudFront-cached path calls `invalidateCloudFrontPaths()` alongside `revalidateTag()`.
...
- `insights-ui/vercel.json` — cron schedules (the `*/3` ones drove invalidations in Phase 1; still fire, now drive CloudFront invalidations via the cache helpers).
...
- **Environment variables** — `NEXTAUTH_URL=https://koalagains.com` (ignored on Vercel but useful for local dev), `COOKIE_DOMAIN` not set, `CLOUDFRONT_DISTRIBUTION_ID=EZI5H8FKNE9R1` (without this the per-save invalidation helper is a silent no-op), plus `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_REGION` for the IAM user `insights-ui-vercel-project`.

### Cache plumbing (partially inert)

See [section 14](#cache-helper-plumbing--what-each-helper-does-today). The `revalidateTag()` half of each helper is inert under `force-dynamic` and can be cleaned up in a follow-up PR. The `invalidateCloudFrontPaths()` half is live and should stay until those URL families are removed from `cloudfront.tf`.
