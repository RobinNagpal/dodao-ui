...
function extractS3Key(s3Url: string): string {
  // Support formats: s3://bucket/key, https://bucket.s3.region.amazonaws.com/key, https://s3.region.amazonaws.com/bucket/key
 ...
 */
function mergeVideosWithStreamCopy(
  inputPaths: string[],
  outputPath: string
): Promise<void> {
  return new Promise((resolve, reject) => {
    console.log(
      `Merging ${inputPaths.length} videos with stream copy (no re-encoding)...`
    );
    ...
    ffmpeg.on("close", (code) => {
      ...
      if (code === 0) {
        ...
        mergeVideosWithReencoding(inputPaths, outputPath)
          .then(resolve)
          .catch(reject);
      }
    });
 ...
 */
function mergeVideosWithReencoding(
  inputPaths: string[],
  outputPath: string
): Promise<void> {
  return new Promise((resolve, reject) => {
...
export async function handler(event: MergeRequest): Promise<MergeResponse> {
  ...
  try {
    ...
    console.log(
      "Using videos directly for maximum speed - no padding processing needed!"
    );
