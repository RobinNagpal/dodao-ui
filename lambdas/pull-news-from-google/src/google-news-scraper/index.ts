...
const googleNewsScraper = async (userConfig: GNSUserConfig) => {
  ...
  const browser = await chromium.puppeteer.launch(puppeteerConfig);
  ...
  $(articles).each(function () {
    ...
    const mainArticle: Article = {
      ...
      datetime:
        new Date(
          $(this).find("div:last-child time")?.attr("datetime") || ""
        )?.toISOString() || "",
  ...
  if (config.getArticleContent) {
    ...
    results = await getArticleContent({
      articles: results,
      browser,
      filterWords,
      logger,
  ...
  return config.limit < results.length
