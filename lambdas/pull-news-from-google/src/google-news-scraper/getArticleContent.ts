...
const extractArticleContentAndFavicon = async ({
  ...
  try {
    ...
    if (!articleContent || !articleContent.textContent) {
      logger.warn("Article content could not be parsed or is empty.", {
        article,
      });
      return { ...article, content: "", favicon };
    }

    const hasVerifyMessage = verifyMessages.find((w) =>
      articleContent.textContent?.toLowerCase().includes(w)
    );
    if (hasVerifyMessage) {
    ...
    if (cleanedText.split(" ").length < 100) {
      // Example threshold: 100 words
      logger.warn("Article content is too short and likely not valuable.", {
        article,
      });
      return { ...article, content: "", favicon };
    }
