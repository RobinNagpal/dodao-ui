...
function parseVoiceConfig(voice?: string): { voiceId: VoiceId; engine: Engine } {
  ...
  const selectedEngine = validEngines.includes(engine as Engine)
    ? (engine as Engine)
    : DEFAULT_ENGINE;
...
export async function generateSlideAudio(
  ...
  try {
    ...
    console.log(
      `Generating audio for presentation ${presentationId}, slide ${formattedSlideNumber} using AWS Polly...`
    );
